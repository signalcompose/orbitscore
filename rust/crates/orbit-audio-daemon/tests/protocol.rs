...
/// PluginAllNotesOff は台帳が空なら冪等に `{released:0, stale:0}` を返す。
#[tokio::test(flavor = "current_thread", start_paused = true)]
...
async fn plugin_all_notes_off_reports_injected_missing_destination_as_stale() {
    ...
    assert_eq!(daemon.engine.active_plugin_note_count(), 1);
    ...
    assert_eq!(daemon.engine.active_plugin_note_count(), 0);
}

/// engine process の異常終了に相当する WebSocket drop では RPC を送れない。session の
/// read loop 終了そのものが同じ all-notes-off 配送関数を起動し、台帳を drain する。
#[cfg(feature = "outproc-instrument")]
#[tokio::test(flavor = "current_thread", start_paused = true)]
async fn dropping_session_drains_active_plugin_note_ledger() {
    ...
    for _ in 0..100 {
        if daemon.engine.active_plugin_note_count() == 0 {
            break;
        }
        advance_and_yield(Duration::from_millis(10)).await;
    }
    assert_eq!(
        daemon.engine.active_plugin_note_count(),
        0,
