...
fn package_oracle() -> Option<PathBuf> {
    let crate_dir =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../rust-spike/clap-test-effect");
    let output = Command::new(crate_dir.join("bundle-macos.sh"))
        .current_dir(&crate_dir)
        .output()
        .ok()?;
    if !output.status.success() {
        eprintln!(
            "CLAP effect oracle packaging failed: {}",
            String::from_utf8_lossy(&output.stderr)
        );
        return None;
    }
    Some(crate_dir.join("target/debug/CLAPTestEffect.clap"))
}
...
fn wait_ready(region: *mut SharedRegion, child: &mut Child) {
    ...
    while unsafe {
        ...
        assert!(Instant::now() < deadline, "CLAP effect child READY timeout");
        std::thread::yield_now();
    }
}
...
fn real_effect_child_restores_and_captures_state_through_the_host_mailbox() {
    ...
    let restore = unique_temp("orbit-clap-effect-restore.state");
    std::fs::write(&restore, expected).expect("write restore state");
    let shm = unique_temp("orbit-clap-effect-mailbox.shm");
    ...
    let sidecar = unique_temp("orbit-clap-effect-captured.state");
