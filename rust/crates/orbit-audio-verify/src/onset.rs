...
pub fn detect_onset_matched(signal: &[f32], template: &[f32]) -> Option<usize> {
    ...
    // map で各 lag の相関和（O(m)）を計算し、max_by で最大の lag を選ぶ。Rust の lazy
    // iterator なので各相関は一度だけ評価される（全体 O(lags·m)）。同点時は max_by の
    // 仕様により「最後」の最大 lag を返すため、テンプレートは非周期な transient を使うこと
    // （持続 sine は自己相関が多峰でピークが曖昧）。
    (0..lags)
        .map(|lag| {
...
/// エンベロープ列が **直線で良く近似できる**（= 形状が線形）かを最小二乗残差で判定する。
///
...
/// 指数減衰（`y = A·r^x`）は振幅領域では非線形 → 正規化 RMSE が大きくなり `false`。
///
/// # 注意（傾きは見ない）
/// 「直線への当てはまりの良さ」だけを見るので、**定数列（傾き 0）も `true` になる**。
/// 「fade が実際に下降しているか」は別途呼び出し側で終端値を assert すること
/// （定数出力 = fade 無しの回帰は、この関数単体では捕まえられない）。
///
/// # 引数
