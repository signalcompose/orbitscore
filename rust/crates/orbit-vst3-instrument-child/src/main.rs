...
fn main() -> Result<()> {
    ...
    if let Some(plugin_id) = &args.plugin_id {
        ...
        eprintln!(
            ...
            orbit_child_runtime::notice::child_info(
                ...
                format_args!("--plugin-id={plugin_id} は VST3 では未使用"),
            )
        );
    }
    ...
    if let (Some(state_path), Some(bytes)) = (&args.state, &state_bytes) {
        // 成功通知。規約と理由は `orbit_child_runtime::notice` に集約してある（#618 / #625）。
        eprintln!(
            "{}",
            orbit_child_runtime::notice::child_info(
                "orbit-vst3-instrument-child",
                format_args!("state restored from {state_path:?} ({} bytes)", bytes.len()),
            )
        );
    }
    unsafe {
