...
export const OUTPUT_LINE_GOLDENS = {
  ...
  noBus: {
    channels: 2,
    /**
     * 2026-09-13 実機（定常状態・8 発の窓）。2 セッションの実測が 11 桁一致した値
     * （`0.12306010509945155` / `0.12306010509933912`）。
     *
     * 🔴 **#921（`#851` B-3 の裁定 D′）で √2 倍に上がった。** 発音側の
     * `equal_power_pan(event.pan)` は `event.pan` が常に 0 だったため、定位ではなく
     * **全 audio への固定 −3 dB** として働いていた。減衰のみの `balance_pan` へ変えて
     * **素材は録音されたレベルで鳴る**ようになった。
     *
     * 旧値 `0.0846173`（2026-09-04）。旧実測は `0.08701663` で、旧 golden 自体が
     * 実測より 2.8% 低い緩い基準だった（許容 12% に収まっていた）。
     * **`0.08701663 × √2 = 0.1230601`** と厳密に一致する。
     */
    rms: 0.1230601,
  ...
  sumOutput: {
    /**
     * 2026-09-13 実機（定常状態・8 発の窓）。実測 `0.12306010509951895`。
     * 🔴 #921（裁定 D′）で √2 倍に上がった（理由は `noBus` の doc）。旧値 `0.0846422`。
     *
     * 🔴 **`noBus` と 11 桁一致する。** 旧 golden では 0.03% ずれていたが、
     * どちらも同じ信号が同じ経路を通るので一致するのが正しい。
     */
    rms: 0.1230601,
    tolerance: RMS_TOLERANCE,
  },
