...
| プリミティブ | librosa の対応 | 独立性の質 | 捕まえるもの |
|---|---|---|---|
| **onset**（本丸） | spectral-flux peak-picking (`onset.onset_detect`) | **アルゴリズム独立** | 真の差分（別アルゴリズム）。検出位置の系統ずれ・取りこぼし |
| **level** (`region_rms`) | `feature.rms`（= `sqrt(mean(x²))`、式は同じ） | **実装独立** | channel stride / 窓境界 / dtype / 正規化 等の**配管バグ**（RMS の定義そのものではない） |
| **pan** (`pan_from_lr_rms`) | 各 ch の `feature.rms` → 等パワー逆算（atan2 は純粋数学・既にアンカーテスト済） | **実装独立** | 各 ch RMS の一致 = atan2 入力の健全性のみ。atan2 の数学は別途 `analysis.rs` のアンカーで pin 済 |

→ onset = アルゴリズム独立 / level・pan = 実装独立。**「RMS/pan の数学を独立に再導出した」とは主張しない。**
...
| メトリクス | 許容 | 根拠 |
|---|---|---|
| onset: ours ↔ scheduled | ±2 ms（≈96 frames @48k） | レンダラに attack fade 無し → ほぼ sample-accurate |
| onset: librosa ↔ scheduled | ±15 ms（≈1.4 × hop512=10.7ms） | spectral-flux の frame 解像度（弱い oracle） |
...
**CI gate にはしない。** committed script + 版固定 `requirements.txt` + 本 README の「Recorded validation」節。
...
`cross_check.py --selftest` で正常系 PASS / 異常系（RMS +50% & onset +15000fr）FAIL 検出を確認済み。
self-test 成果物は `.gen/_selftest.pcm` 等に作って実行後に削除する（本物の `.gen/<fixture>.pcm` は巻き添えにしない）。
