...
describe('quantizeToNextBar', () => {
  it('shared uses global bar, independent uses sequence bar', () => {
    ...
    // bar durations
    const barShared = barDurationSeconds({ ...seq, config: { ...seq.config, meter: { n: 5, d: 4, align: 'shared' } } }, ir);
    const barIndep = barDurationSeconds({ ...seq, config: { ...seq.config, meter: { n: 5, d: 4, align: 'independent' } } }, ir);
    expect(barShared).toBeCloseTo(2.0, 3); // 4/4 @120 -> 2.0s
    expect(barIndep).toBeCloseTo(2.5, 3);  // 5/4 @120 -> 2.5s

    // quantize
    expect(quantizeToNextBar(2.1, { ...seq, config: { ...seq.config, meter: { n: 5, d: 4, align: 'shared' } } }, ir)).toBeCloseTo(4.0, 3);
    expect(quantizeToNextBar(2.1, { ...seq, config: { ...seq.config, meter: { n: 5, d: 4, align: 'independent' } } }, ir)).toBeCloseTo(2.5, 3);
  });
});
