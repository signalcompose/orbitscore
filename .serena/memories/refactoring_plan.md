...
2. **sequence.ts** (571行) - ⏳ **進行中** (Issue #26、ブランチ: `26-refactor-sequence-phase-4-4`)
3. ~~**supercollider-player.ts** (506行)~~ ✅ **完了** (PR #23マージ済み)
4. ~~**global.ts** (432行)~~ ✅ **完了** (PR #24作成中)
...
#### 4-3: global.ts (432行) ✅ **完了** (PR #24作成中)
- **Issue**: #24
...
- **リファクタリング結果**:
  ...
  - `effects-manager.ts` (200行) - マスターエフェクト管理
...
- **リファクタリング方針**:
  ...
  - `state/` - 状態管理
- **責務分析**:
  1. **パラメータ管理** (~200行) - `gain()`, `pan()`, `tempo()`, `beat()`, `length()`, `audio()`, `chop()`
  2. **スケジューリング** (~200行) - `scheduleEvents()`, `scheduleEventsFromTime()`, `run()`, `loop()`, `stop()`
  3. **状態管理** (~100行) - `getState()`, `setName()`, `mute()`, `unmute()`
  4. **その他** (~70行) - コンストラクタ、ヘルパーメソッド
- **推奨ディレクトリ構造**:
  ```
  packages/engine/src/core/sequence/
  ├── types.ts (型定義)
  ├── parameters/
  │   ├── gain-manager.ts
  │   ├── pan-manager.ts
...
- **Phase 4-3**: ✅ **完了**（PR #24作成中）
- **Phase 4-4**: ⏳ **進行中**（sequence.ts、Issue #26）