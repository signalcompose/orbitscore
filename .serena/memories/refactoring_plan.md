...
#### 2-1: audio-slicer.ts (139行)
- **現状**: オーディオファイルのスライス処理
- **課題**: 
  - 単一ファイルに複数の責務（ファイル操作、sox実行、一時ファイル管理）
  - エラーハンドリングが散在
- **リファクタリング方針**:
  - `audio/slicing/` ディレクトリを作成
  - `slice-audio-file.ts` - メイン処理
  - `sox-command.ts` - soxコマンド実行
  - `temp-file-manager.ts` - 一時ファイル管理
- **テスト**: 既存のテストを維持、追加テストを作成

#### 2-2: timing-calculator.ts (151行)
- **現状**: タイミング計算ロジック
- **課題**:
  - 複数の計算ロジックが1ファイルに集約
  - テストが困難な部分がある
...
1. **ブランチ作成**: `feature/refactor-<filename>`
2. **リファクタリング実施**:
   - 既存のテストが通ることを確認
   - コードを分割
   - 新しいテストを追加
   - すべてのテストが通ることを確認
3. **コミット**: 小さな単位で頻繁にコミット
4. **PR作成**: developブランチに対して
5. **レビュー**: 変更内容を確認
6. **マージ**: squashマージ
7. **次のファイルへ**
