...
## DSL Code Writing (最重要)

**MUST read specification BEFORE writing any DSL code:**

### 必須ルール

1. **仕様書の事前確認**
   - `.osc`ファイルやDSLコードを書く前に、**必ず** `docs/INSTRUCTION_ORBITSCORE_DSL.md` の該当セクションを読む
   - 推測でコードを書かない

2. **既存例の参照**
   - パターンやコードを作成する前に：
     1. `examples/`ディレクトリで類似パターンを確認
     2. DSL仕様書で使用するメソッドを確認
     3. パラメータの順序、型、期待値を検証
     4. 各メソッドの目的を理解（例: `beat()`は拍子、リズムパターンではない）

3. **よくある間違い**
   - ❌ `beat("x___")` - `beat()`はリズムパターンではなく拍子設定
   - ✅ `beat(4)` - 4/4拍子
   - ❌ `play("1")` - 文字列ではなく数値を使用
   - ✅ `play(1, 0, 0, 0)` - リズムパターンは`play()`で定義

4. **構文の確認**
   - 新しいメソッドや構文を発明しない
   - 仕様書に記載されている構文のみ使用
   - 不明な点は仕様書を確認、それでも不明ならユーザーに質問
...
global.tempo(120)      // BPM設定
global.beat(4)         // 4/4拍子
global.tick(4)         // tick resolution

// シーケンス作成
var seq = init global.seq
seq.length(1)          // 1小節ループ
seq.audio("path.wav").chop(4)  // 音声を4分割
seq.play(1, 2, 3, 4)   // リズムパターン（0=休符、1-n=スライス番号）
...
5. **未知の仕様や技術**
   - 未知の仕様や技術、参照先などが出てきた場合
   - ドキュメントに記載がない技術仕様
   - 新しいツールやライブラリの使用方法が不明
