...
async function main() {
  ...
  const hist = await slackApi('conversations.history', { channel, limit: 100, include_all_metadata: true });
  ...
  console.log(JSON.stringify({ channel, botUserId, count: mentions.length, mentions: mentions.reverse() }, null, 2));
}
