...
const token =
  process.env.ORBITSCORE_SLACK_BOT_TOKEN || process.env.SLACK_BOT_TOKEN;
const channelPref = (
  process.env.ORBITSCORE_SLACK_CHANNEL ||
...
if (!token) {
  console.error(
    "Missing ORBITSCORE_SLACK_BOT_TOKEN (or SLACK_BOT_TOKEN) in .env",
  );
  process.exit(1);
}
...
async function resolveChannelId(input) {
  ...
  if (/^C[A-Z0-9]+$/.test(name)) return name;
...
async function main() {
  ...
  const hist = await slackApi("conversations.history", { channel, limit: 5 });
  const out = (hist.messages || [])
    .map((m) => ({ ts: m.ts, user: m.user, text: m.text }))
    .reverse();
  console.log(JSON.stringify({ channel, messages: out }, null, 2));
}
