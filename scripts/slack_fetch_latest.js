...
async function resolveChannelId(input) {
  ...
  const list = await slackApi('conversations.list', { limit: 200, exclude_archived: true, types: 'public_channel,private_channel' });
...
async function main() {
  ...
  const out = (hist.messages || []).map(m => ({ ts: m.ts, user: m.user, text: m.text })).reverse();
