...
| **PR-O3b** 🔴 | `feat(daemon): SetBusLine wire + TS client` | #611 §4.1 wire 契約と検証表・`protocol-types.ts` / `daemon-client.ts`。🔴 **TS の呼び出し元は作らない**（DSL から送るのは PR-O4）| `session.rs`（+180 検証）・`engine_wrap.rs`（+60）・`protocol-types.ts` `daemon-client.ts`（+60）・cargo test（+200）| PR-O3a | cargo: 検証表の各行 + **全か無か** + 旧 `SetBusRouting` が壊れていないこと | W-1 |
