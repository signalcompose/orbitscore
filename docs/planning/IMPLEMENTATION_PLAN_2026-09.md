...
| PR-O3（🔴 **2026-09-08 に O3a / O3b へ分割**・下 2 行が正）| `feat(daemon): LineProgram + SetBusLine (full-replacement bus line wire)` | #611 wire・#647 shm 拡張。🔴 **束 O-wire**（`611-line-wire`）に **#773 と一緒に**入れる。**検証は「goldens が動かないこと」** — 互換維持の配線入れ替えなので、この検算はこの束でしか使えない | `output.rs`（+350: `OutputDest`/`LineOp`/`LineSlot`・RT 実行）・`session.rs`（+180 検証）・`protocol-types.ts` `daemon-client.ts`（+60）・cargo test（+200）| PR-O2 | cargo: forward-only 検証・thru:false で break・ramp。実機: 既存譜面が `SetBusRouting` 経路のまま同音（両 wire 併存）| W-1 |
| **PR-O3a** 🔴 | `feat(daemon): line program and RT execution, legacy SetBusRouting mapped onto it` | #611 §5.1 型・§5.3 RT。**旧 `SetBusRouting` を内部で program 生成へ写して互換維持**。🔴 **束 O-wire**（`611-line-wire`）| `output.rs`（実測 +1109/-131）・`engine_wrap.rs`（+206）| PR-O2 | cargo: forward-only・`thru:false` で break・ramp・**互換の f32 bit 一致（4 トポロジ）**。実機: **goldens が動かないこと** | — |
| **PR-O3b** 🔴 | `feat(daemon): SetBusLine wire + TS client` | #611 §4.1 wire 契約と検証表・`protocol-types.ts` / `daemon-client.ts`。🔴 **TS の呼び出し元は作らない**（DSL から送るのは PR-O4）| `session.rs`（+180 検証）・`engine_wrap.rs`（+60）・**`output.rs`（`MasterLine.line`・下記）**・`protocol-types.ts` `daemon-client.ts`（+60）・cargo test（+200）| PR-O3a | cargo: 検証表の各行 + **全か無か** + 旧 `SetBusRouting` が壊れていないこと + **master 未 publish 時の bit 一致** | W-1 |
| PR-O4 🔴 | `feat(dsl): output(dest, thru, db) / send in dB / pan as a line element — AudioLine on Sequence and master` | 🔴 **束 O-surface**（`611-output-line`）。**daemon 台数のアサーション（#624 の狭いスライス）をこの PR に含める** — 検証列のとおり **E2E-10（daemon respawn）は本 PR の検証**だから。#611 DSL・#649 実装 B・**#543-a 差分ゼロ（`pan` 譜面は再ベースライン）**・同一宛先の複数 `output`（2 要素）・mono 宛先。🔴 **wire も本 PR で足す**（owner 裁定 2026-09-10）: `pan` op と mono `device`（`channels` 1 要素）。設計 §4.1 が 09-03 の裁定（Q-611-4 / Q-611-5）に追従しておらず **PR-O3b で落ちていた**（§4.1 の引用ブロックに経緯）。`pan` は **RT 実行も無い**ので `validate_line_program` の拒否を外して `Pan` op を実装する。**`SetGlobalGain` の master line への写しも本 PR**（§4.2・ramp の実効値引き継ぎ機構とセット） | `core/sequence/audio-line.ts`（新 +240）・`sequence.ts`（±130）・`global.ts`（+40）・`mixer-manager.ts`（±60）・`runtime.ts`（±30）・`evaluate-method.ts`（+40）・`repl-mode.ts`（+20）・**`session.rs` / `engine_wrap.rs` / `output.rs`（wire + RT の `Pan`・+150）**・tests（+320）| PR-O3・**PR-L2（フレーム）**・PR-O1 | E2E-2〜7・E2E-10 + `pan` の L/R（PR-E3 の後）。実機: `kick.output(verb, thru:true, db:-12).output(master)` を評価して aux の RMS 比。🔴 **束 O-wire の締めで測定した「実機で一度も鳴っていない」op をここで押さえる**（doc 611 §10 冒頭）: `LineOp::Gain` / `Device` 宛て / **mono マージ** / **複数 send** / **gain 0 での send 除去** | W-2 / W-3 / W-18 |
...
> 🔴 **PR-O3b が `output.rs` も触る理由**（2026-09-09 に実装中の実測で判明・main の裁定）:
> 設計 [doc 611](../design/611-output-line-design.md) **§5.2 は `MasterLine` に `line: LineSlot` を持たせる**と定めており、
> **§4.1 の `bus` は `"master"` を受理対象にしている**（`bus == "master"` の自己参照を拒否する検証行がその証拠）。
> ところが **PR-O3a は `MasterLine` に `line` を入れなかった** — main `2ca00f6a` の `output.rs:710-728` の
> `MasterLine` は `buffer` / `direct_device_buffer` / `post` / `gain_target` / `gain_current` / `ramp_frames` だけで、
> master は従来どおり「post → gain」の固定経路のまま。**`SetBusLine("master", …)` を受理する先が無い。**
>
> したがって O3b が §5.2 の `MasterLine.line` を埋める。**上の「触るファイル」欄が `output.rs` を落としていたのは
> 見積もりの漏れ**であって、設計の変更ではない。
>
> 🔴 **束 O-wire-b の収束条件（goldens が 1 つも動かない）に直結するので、次を検証に足す**:
> **master line が未 publish の間は従来経路と bit 一致**であること。既定 program は §5.2 のとおり
> `[Rack, Output(Device{0,1}, thru:false, 1.0)]` で、publish されるまで固定互換経路を実行する。
...
> | **#756** ✅ | `setupStderrHandler` の `ERROR:` 前置が **chunk 単位**。同じ chunk の 2 行目以降に前置が付かず、`countErrors` が**構造的に過小カウント**する | `createLinePrefixer()` を足して行単位へ（部分行の持ち越し・`'end'` の `flush()`・空行スキップ）。🔴 起案時の「`createDaemonStderrLineRouter` が同じ問題を解いている」は**半分だけ正しい** — engine 側の router は部分行を持ち越すが**終端の flush を持たない**（`packages/engine/src/audio/rust-engine/daemon-client.ts:167-182` に flush が無く、`:906-922` の呼び出し側も `end` で drain していない） | **PR [#772](https://github.com/signalcompose/orbitscore/pull/772) で対処済み**（束 `761-gated-measurement` 経由で **main へマージ済み**・merge commit `d2e94af7`） |
...
| **E-gate** 🔴 | `780-merge-gate` | PR-E10・E14・E13 | 約 210 行（doc 668 §13.5.3。✅ **完了**（2026-09-07・PR [#789](https://github.com/signalcompose/orbitscore/pull/789)）。ステージ 2 は着手済み。収束条件: **#780 が 10 回連続で緑** + 実行前後で shm が増えない）|
| ~~**E-router**~~ | ~~`777-line-router`~~ | ~~PR-E12~~ | 🔴 **束としては持たない**（2026-09-07・§3「安全網の残余」）。**#773 はステージ 2 の 1 束目（O-wire）で解決済み**（2026-09-08・PR #811）、**#777 は backlog**。doc 668 §13.5.2 の収束条件（「chunk 列 → 行」を 1 本の共有プリミティブへ）と **#757 の直前**という順序制約は、**#757 に着手する時点で #777 と一緒に畳む**形で保たれる（§3 末尾）|
...
> 🔴 **2026-09-10 裁定（owner・#827）: 拡張版 stable の凍結線は O-surface（PR-O4）完了。**
> 正本は [`NATIVE_MIGRATION_2026-09.md`](NATIVE_MIGRATION_2026-09.md) §12。
>
> | 束 | 状態 | 凍結版 |
> |---|---|---|
> | O-wire（PR-O3a + #773 + #801） | ✅ #811 | 含む |
> | O-wire-b（PR-O3b） | ✅ #824 | 含む |
> | **O-surface（PR-O4）** | 次 | **含む — ここが凍結線** |
> | O-multiout（PR-O5 + PR-O6） | — | **含まない → 新ラインの最初の束**。PR-O6 の「O4 が実機で確かめられた後」は stable 版の制作利用が満たす |
>
> 制作の要件は master + sum / aux + 物理アウト（`Device` 宛て・O4 に含まれる）で足りる。`outs:`（#409 / #647）は不要。
> したがって **#611 / #409 / #647 が閉じるのは新ライン**で、凍結版では閉じない。
...
✅ **#801（CI の flaky）は束 O-wire で解決済み**（2026-09-08・PR #811・グローバル anchor subscriber）。🔴 **「負荷依存」は 2026-09-08 の実測で否定された** — 効く変数は `--test-threads`。`device_switch_result_records_failure_and_success_through_the_same_path` が `captured log: ""` で間欠的に落ちる（2026-09-07 に CI で発生・**docs のみの PR #800 で**）。`rust-ci.yml` は**全 PR で走る**ので、赤の帰属ができなくなり慣れるとゲートが死ぬ — **#780 をステージ 2 の着手条件にしたのと同じクラス**。O-wire は Rust を触る束なので**この CI ジョブを最も多く回す**。
