...
**制定**: 2026-09-11（起案: Fable subagent〔effort: xhigh〕/ 検収: main / 裁定: owner・#848）/ **改訂**: 2026-09-18（Fable〔effort: high〕・main `1276ab1f`・拡張 4.2.0 凍結後）
**位置づけ**: **凍結線を越えた後の開発計画の正本。** 改訂: 凍結線は初版の「O-surface 完了」から #883 で 4.0.0 へ動き、**拡張 4.2.0 で凍結された**（owner 2026-09-18・`packages/vscode-extension/package.json`・PR #943）。**この地図の期間は始まっている。** 凍結版は [`DEVELOPMENT_MAP.md`](DEVELOPMENT_MAP.md)（拡張版の地図）が正本のまま。**両者は同時に生きる**が、主語が違う（拡張版 = 凍結と `.vsix` の保守 / 本地図 = **OrbitStudio**〔= ネイティブ版アプリ・owner 用語確定 2026-09-18〕と層 1 / 2 の続き）。文書（user / dev サイト）の保持構造は [`848-docs-structure-design.md`](../design/848-docs-structure-design.md)（別冊）。
...
| 凍結線と新ラインの順序 | 凍結線 = O-surface → **4.0.0（#883）→ 4.2.0 で凍結済み** / O-multiout が新ラインの最初の束 / ステージ 3〜7 は生きる / ステージ 8 は再定義 / 凍結の翌日から新ライン（**= 今**） | 同 §12・`DEVELOPMENT_MAP.md:236-241`・PR #943 |
...
```
  ┌── 凍結版（旧地図の領分）✅ 完了（2026-09-14 4.2.0・owner 凍結確定 2026-09-18）──────┐
  │  O-surface（PR-O4）✅ ── SC 削除（#502）✅ ── README 導線（#937）✅ ── #883 → 4.0.0 ✅ ── 4.2.0 ✅ │
  └────────────────────────────────────────────────────────────────────────────────────┘
        ↓ 凍結の翌日から（ブランチも実機も共有しない）— ここが現在地
  ...
  [G] docs             文法 + frontmatter（別冊 §5）─→ 章の分類と併記（別冊 §3）─→ ルーチン手順（別冊 §7）─→ 同梱（R-app N4-e・D-9）
                          （主語の書き換えは A-parity 後・設計 §14 (20)。「アプリ主語へ全面改訂」は改訂で撤回）
...
### 3.1 現在地（2026-09-18・main `1276ab1f`・改訂で再測）

| 領域 | 現在地 | 証拠 |
|---|---|---|
| ネイティブ線の成果物 | **何も無い**。`apps/` `protocol/` `packages/session/` は存在しない | `ls -d apps protocol packages/session`（2026-09-18） |
| 設計・計画・地図 | 📐 3 本（初版 09-11・改訂 09-18）+ 別冊（文書構造） | `docs/design/848-…` / `docs/planning/IMPLEMENTATION_PLAN_NATIVE.md` / 本書 / `docs/design/848-docs-structure-design.md` |
| gated ハーネスの起動先 | ✅ stock VS Code（フォーク非依存） | PR #831 / `orbitstudio-mcp-gated.spec.ts:757,763` |
| フォークのビルドスクリプト | ✅ 削除済み | 同上 |
| 凍結 | ✅ **拡張 4.2.0 で凍結**（O-surface ✅ / #502 SC 削除 ✅ / README 導線 #937 ✅ / #883 = 4.0.0 ✅ / 4.1.0 pan ✅ / 4.2.0 #939・#940 ✅） | `packages/vscode-extension/package.json` `version` / PR #943 / owner 2026-09-18 |
| 拡張の規模 | **40 ファイル 10,471 行**（#887 で `extension.ts` 4,258 → 412・`mcp-server.ts` 1,417 → 346）。500 行超は 3 本 | `wc -l packages/vscode-extension/src/*.ts` |
| 層 2 の抽出可能性 | `vscode` 非依存は **26 ファイル 6,062 行**、依存は **15 ファイル 4,409 行**。🔴 非依存 = 移す、ではない: **移す 17 本 4,290 行 / 消える 8 本 1,499 行（ブリッジ・child-engine の stdout 経路）/ WebView へ 1 本 273 行**。依存側からも約 800 行の意味論（補完・subject-block・カーソル下プラグイン解決・`*ForAgent`）が層 2 へ落ちる | `grep -l "from 'vscode'" packages/vscode-extension/src/*.ts` / 設計 §3.1 |
| MCP 層 | **8 ファイル 1,658 行が pure**。ツール **26**（`force_kill_scsynth` は消え `open_plugin_ui_at_cursor` が増えた）。登録は `mcp-tools-*.ts` 3 本、実装は **`OrbitScoreToolHandlers`** 1 インタフェース | `grep -c "registerTool(" packages/vscode-extension/src/mcp-tools-*.ts` / `mcp-types.ts:176-224` |
| プロトコルの現行 | `//#` メタ行 **8 種**（`evalBegin` / `evalEnd` が O-surface で増えた）+ stdout 3 系統。相関ブリッジ **5 本 654 行**（#757 OPEN・6 本目は無い）。daemon protocol **v0.3** | `grep -rho '//#[a-zA-Z]*' … \| sort -u` / `wc -l …/*-bridge.ts` / `protocol.rs:8` |
| engine の spawn | **VS Code 同梱の Node**（`process.execPath` + `ELECTRON_RUN_AS_NODE`・#878）。PATH の node に依存しない | `engine-process.ts:382-416` |
| gated E2E | **49 `it` / 48 passed・skip 0**（2026-09-14）。suite 7,393 行。cold-install E2E あり（`vsix-cold-install-gated.spec.ts`・#873） | `WORK_LOG.md:452` / `grep -cE "^ +(it\|test)…"` |
| 文書の同梱 | 🔴 **凍結版は docs を同梱していない**（`.vscodeignore` が `../../**` を除外）。Docs パネル / `get_dev_doc` はモノレポ前提（`__dirname/../../..`）で **cold install では動かない**。gated E2E も 0 本 | `.vscodeignore:20,28` / `mcp-server.ts:137-138` / `mcp-docs.ts:29-36` |
| 文書サイトの現状 | user 21 章（ja・153 KB）/ dev 27 章（ja・1.19 MB）・両方 ja/en。`deploy-sites.yml` が **main の push で** GitHub Pages へ（= サイトは常に main = 未リリース含む）。VitePress の版分岐は無し（`base` 固定） | `sites/*/.vitepress/config.ts` / `.github/workflows/deploy-sites.yml:15-22` |
| 署名の材料 | Developer ID `ZWULF5LA37`・ASC API キーは手元にある。entitlements は未確認 | `656-release-design.md` §5.2-5.3 |
| 未検証（裁定 §9） | Monaco × WKWebView × **IME** / スキーマ生成ツール / 同梱 node の hardened runtime（#878 で Electron 同梱 Node が N-API を読めた傍証あり・素の node は未実測）/ 層 2 の多クライアント同時性（**実測は S-mcp**）/ Swift の CI | `NATIVE_MIGRATION` §9 / `engine-process.ts:392-396` |
...
| N3 A-panels | 👀 | デバイス選択・engine の起動停止・ログ・プラグインカタログ・**同梱 docs（オフライン）**がアプリの中で見える。右クリックでその 1 つのプラグイン UI が開く。flash と playhead の色が設定できる |
...
| MCP を層 2 へ（ツール **26 のまま**・中身は `force_kill_scsynth` → `open_plugin_ui_at_cursor` に入れ替わり済み） | 設計 §6 / 計画 S-mcp | ○（改訂: MCP 層 **8 ファイル 1,658 行が pure**・`OrbitScoreToolHandlers` を層 2 で実装する形・実測 2026-09-18） | S-rpc |
| document 同期・診断・補完・subject-block・カーソル下プラグイン解決（層 2 の言語サービス） | 設計 §5.4-5.5 / 計画 S-editor | ○（`diagnostics-analysis.ts` / `plugin-name-diagnostics.ts` は pure。`completion-context.ts` 290 行・`run-selection.ts` の `getLineSubject`・`plugin-ui-at-cursor.ts` の解決部が vscode 依存を剥がす対象） | S-mcp |
...
| daemon protocol（層 1） | `protocol.rs:8` | **v0.3**（改訂: 初版の v0.2 から engine 線で進んだ）。アプリ線は触らない（裁定 2.4）。engine 線の束が進める。本地図の対象外 | — |
...
| cold-install E2E（成果物に gated `native`・PATH を絞る） | 設計 D-7 / 656 §6 | ○（改訂: 凍結版には同型の `npm run test:e2e:cold-install` が在る・#873 / #878。その `.app` 版） | R-app |
| docs 同梱（`Contents/Resources/docs/`・D-9） | 別冊 §6 / 計画 R-app N4-e | ○（凍結版は同梱無し・`.vscodeignore:20,28`） | owner 裁定 (15)(16) |
...
| `native-parity-ledger.ts`（凍結タグの `it` 題名を全列挙・未分類 red） | 設計 D-1 / 計画 S-mcp | ○（改訂: 母集団は **4.2.0 の 49 `it`・48 passed / skip 0**〔2026-09-14・`WORK_LOG.md:452`〕。docs 系ツール 2 本は凍結版に E2E が無いので別枠） | 凍結タグ（4.2.0・✅） |
...
### 4.G docs（改訂 2026-09-18・正本は別冊 [`848-docs-structure-design.md`](../design/848-docs-structure-design.md)）

| 段 | 正本 | 現在地 | 前提 |
|---|---|---|---|
| 可用性表記の固定文法 + frontmatter `host:` + lint（`check-availability.mjs`） | 別冊 §5 | ○ | owner 裁定 (16) |
| user サイト全章の分類（host 依存段落の併記・機能の可用性行） | 別冊 §3 の表 | ○（🔴 「アプリ主語へ全面改訂」は**撤回**。主語は A-parity 後・(20)） | 上の文法 |
| docs 追従ルーチンの手順更新（PR マージ維持 + 「未リリース（OrbitStudio）」マーカー・暫定行の扱い） | 別冊 §7 | ○（ルーチンは PR マージトリガーで稼働中・`docs/archive/WORK_LOG_2026-09.md:264` 等） | owner 裁定 (18) |
| docs 同梱（`build-docs-bundle.mjs`・`manifest.json`・`verify-app.sh` の検査・D-9） | 別冊 §6 / 計画 R-app N4-e | ○ | (15)・R-app |
| 凍結スナップショット（`/orbitscore/ext/`・タグから静的配信） | 別冊 §4.3 | ○・**出さない**（推奨 A・(17)） | 凍結版の bug fix リリースが出た時に再検討 |
| dev サイト `sites/dev/editor/`（`vscode-architecture.md` 1,197 行ほか）にネイティブ線の章 | dev STYLE_GUIDE | ○ | S-rpc（プロトコルが確定してから） |
| `CLAUDE.md` を層ごとに（`packages/session/` `apps/OrbitStudio/` `protocol/`・中身は「置かないもの」だけ） | 設計 §8 | ○ | 各束の最初の小 PR で |
| CLAUDE.md マージ前ゲートに `swift build` を足す | 設計 §9.4 | ○（docs・main 直行） | N0-a で sandbox の可否を確認してから |
...
| ~~拡張が同梱している docs（`get_dev_doc` の資産）の実体と、アプリでの同梱形~~ | ✅ 改訂で確定: **同梱していない**（`.vscodeignore:20,28`・`mcp-server.ts:137-138` はモノレポ前提）。アプリでの同梱形は別冊 §6 |
| 同梱 docs の Markdown を WKWebView / 層 2 の HTTP でどう配信するか（HTML と Markdown の二重同梱で足りるか） | N3 Docs パネルの最初の小 PR（別冊 §6.2 の形で始め、サイズを `verify-app.sh` で記録） |
| docs 追従ルーチンの実行制限（PR マージ 1 件あたりの消費）が新ライン込みで足りるか | ルーチンの実行履歴（cloud 側・リポジトリ外・**確認できず**） |
...
| 2026-09-18 | 改訂（Fable・effort high・main `1276ab1f`）。凍結を ✅（4.2.0）に。§3.1 現在地を再測（40 ファイル / 15 依存 / MCP 層 8 本 / メタ行 8 種 / ブリッジ 5 本 654 行 / protocol v0.3 / gated 49 `it` / #878 の spawn / docs 未同梱）。§2 [G] と §4.G を別冊（文書構造）の順序に置き換え、「アプリ主語へ全面改訂」を撤回。§7 の docs 行を確定に |
