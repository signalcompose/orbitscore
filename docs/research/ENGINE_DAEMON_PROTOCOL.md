...
- クライアントは stdout の ready line を待つタイムアウト（**10 秒**）を設け、期限内に届かなければ stderr を確認し、プロセスを kill しエラー通知。
...
| **パラメータ検証厳密度** | MEDIUM | #107 実装時 | gain / pan が範囲外のとき clamp するか `PARAM_OUT_OF_RANGE` で reject するか |
