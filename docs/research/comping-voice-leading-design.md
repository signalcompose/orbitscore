...
`.r` / `^r` は**ツリーを固定したまま**発音/オクターブだけを毎サイクル再ロールする。`.comp` のリズム生成は **リズムの subdivision 自体が dispatch-time で可変になる初のケース**である（構造そのものが静的でなくなる）。これは「`*n`/spread の eval 時展開」とは別物で、本機能の唯一の真の新規アーキテクチャ点。実装時はここに設計の重心を置く（dispatch パイプラインが可変サブディビジョンを扱えるか、`.orbslog` が展開後の具体リズムをどう記録するか）。
