...
## Part 3 — 確定した方針（2026-06-14, ユーザー確定）

実装に入るにあたり以下を確定（旧「未決事項」を解決）:

1. **進め方**: **C1 → C2 → C3 の段階実装**。各段で PR + /simplify + レビュー。C1 から着手。
2. **命名**: auto voice-leading は **`.voicelead()`**（主）+ **`.vl()`**（alias）。両方を提供。
3. **呼び出し/入力面**: **シーケンス既定 と グループ単位の両方**に対応（`.root()`/`.oct()` と同様、`seq.voicelead()` も `(...).voicelead()` も可）。`.comp(changes)` も seq / group 両対応。対象はコード stack `[ ]` 列、単音はスルー。
4. **リズム生成（C2）**: **ハイブリッド**（subdivision grid primitive + 名前付きセル ライブラリ、mode と同形）。
5. **seed**: #53 準拠（seed なし・毎サイクル再ロール）を維持。
6. **`.voicelead()` の音楽性限界**: 「デフォルト滑らか（L1 最小）+ ユーザー制御」で割り切る。傾向音解決・並行回避は保証しないことを明記する。

### 追補（2026-06-14, C2a 実装中にユーザー確定）

7. **セルは meter 非依存の固定分割**（旧「meter 由来 slotsPerBar + 収まり判定/density フォールバック」を棄却）。各セルは固有スロット数（Charleston=8 等）を持ち、小節をその数で等分割する。偶数グリッドのセルを奇数拍子に乗せたときの「ズレ」は**意図的なポリメーター**（8:3 等）として歓迎する設計とする（多層時間構造と掛け算可能）。→ 正本 PITCH_DSL_SPEC §6.4。
8. **音価は `gate`、off は rest**（調査根拠: 標準コンピングは Freddie Green 的に短い／pad・legato 持続は別スタイル）。タイ持続（次 onset まで伸ばす）は将来オプション。
9. **コンピング知能（voicing 自動選択・rootless A/B・密度連動 sustain = 旧 C3）は DSL 関数にしない**。OrbitScore は**メカニズム/primitive**（`.comp` セル/density・`.voicelead`・voicing 演算子・gate・タイ）に徹し、**どう組むかの音楽的判断は LLM バンドメイト側スキル**が持つ。理由: DSL に知能を詰めるほど「生成 AI が DSL ベースで音楽理解を発揮するバンドメイト」という目標と逆行する（記録済み哲学「ユーザー制御が主役・自動作曲ではない」と整合）。→ §2.6 の C3 行は「DSL スコープ外（LLM スキルへ移管）」に改める。

10. **C2b の「確率的 per-cycle サブディビジョン再ロール」は棄却**（2026-06-14 ユーザー確定）。リズムの揺れ/進化は **多層時間構造（ポリリズム/ポリメトリック/ポリテンポ/入れ子分割）で決定論的に出す**ものとし、**時間グリッド（いつ鳴るか＝groove の timing）に計算的乱数を入れない**。
    - **研究的理由**: RNG が混ざると groove が構造由来か乱数由来か分離できず、「多層時間構造がどう groove を生むか」の研究実験がコンタミする。
    - **哲学的理由**: 本 DSL の主張は「groove は構造として compose する＝制御可能・説明可能」。時間グリッドへの RNG はその主張自体を掘り崩す。
    - **代替（既に可能/一部実現済）**: meter 非依存セルを奇数拍子へ（決定 #7）＝ポリメーターで決定論的にループが phase（例: charleston/5-4 は 8:5 で 5 小節周期）。ポリテンポで ensemble に対し phasing。UC4「ループが機械的にならない」は RNG なしで達成。
    - **境界（軸の分離, 2026-06-14 確定）**: 本原則は **groove / 時間感（いつ鳴るか）** の軸に限る。乱数の可否は軸で分かれる:
      - **groove(timing) 軸**: 決定論的・帰属可能であるべき → RNG 禁止（C2b 棄却の根拠）。`swing`/`anticipate` は決定論的 timing 変換なのでこの軸に乗せて OK（将来候補）。
      - **偶発 / chance 軸（どの音が鳴るか・オクターブ）**: `.r`/`^r`（決定 #53）。**揺らすためでなく偶発を生むためのもの**で、意図的に非決定的（それが価値）。時間グリッドを動かさないので本原則と**直交し、維持する**（旧「要検討」を解決＝残す）。

> 上記 7-10 は本ドラフト Part 2 の提案を上書きする確定事項。次回 DESIGN_DISCUSSION_RECORD へ正式な決定番号で昇格する。
