...
9. **コンピング知能（voicing 自動選択・rootless A/B・密度連動 sustain = 旧 C3）は DSL 関数にしない**。OrbitScore は**メカニズム/primitive**（`.comp` セル/density・`.voicelead`・voicing 演算子・gate・タイ）に徹し、**どう組むかの音楽的判断は LLM バンドメイト側スキル**が持つ。理由: DSL に知能を詰めるほど「生成 AI が DSL ベースで音楽理解を発揮するバンドメイト」という目標と逆行する（記録済み哲学「ユーザー制御が主役・自動作曲ではない」と整合）。→ §2.6 の C3 行は「DSL スコープ外（LLM スキルへ移管）」に改める。C2b（per-cycle 可変 subdivision）はメカニズム寄りのため保留（やるなら DSL、WCTM クリティカルパス外）。

> 上記 7-9 は本ドラフト Part 2 の提案を上書きする確定事項。次回 DESIGN_DISCUSSION_RECORD へ正式な決定番号で昇格する。
