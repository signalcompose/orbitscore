...
10. **C2b の「確率的 per-cycle サブディビジョン再ロール」は棄却**（2026-06-14 ユーザー確定）。リズムの揺れ/進化は **多層時間構造（ポリリズム/ポリメトリック/ポリテンポ/入れ子分割）で決定論的に出す**ものとし、**時間グリッド（いつ鳴るか＝groove の timing）に計算的乱数を入れない**。
    ...
    - **境界**: `swing`/`anticipate` は**決定論的 timing 変換**なので本原則と矛盾せず、将来の groove-shaping 候補として残す。`.r`/`^r`（presence/octave の opt-in 乱数, 決定 #53）は時間グリッドを定義しないので別扱いで残す（「同じレンズで `.r` を見直すか」は要検討としてのみ記録、現状変更なし）。
