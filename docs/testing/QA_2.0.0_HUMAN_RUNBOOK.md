...
> 🔴 **2026-09-10 追記**: SuperCollider 経路は **#502（2026-09-10）で削除**され、現在のバックエンドは Rust `orbit-audio-daemon` 単独。`SuperCollider` / `OrbitLinkAudio.scx` に言及する手順（§4b Link Audio 等）は**当時の実機確認の記録**であり、今日そのまま再実行することはできない。
