...
**既知の弱点（正直に）**: TS の正規表現判定は発見的（heuristic）である。`a / b` の除算と `/re/` の区別を直前トークンで決めるので、`x = y\n/re/.test(z)` のような ASI 依存の書き方は誤判定しうる。誤判定の帰結は「文字列状態のまま終端に達する」= §3.3 の例外で **red になって気づく**（黙って通らない）。
...
|---|---|---|---|
| Rust | `:(glob)rust/crates/**/*.rs` | `**/tests/**` `**/examples/**` `**/benches/**` `**/build.rs` `**/src/**/tests.rs` | 96（= 149 − tests 50 − examples 2 − build.rs 1 − `src/tests.rs` 1。`src/bin/` 6 件と workspace 外の `orbit-link-audio` 3 件を**含む**） |
...
4. **スクリプトは子 0b に入れる**（子 0 は測定だけ・分割の道具は持たない。子 1 の前に要る点は関数 lint と同じ）。中身は §13.3 のコマンドを `-c color.diff.*` で色を固定して呼び、ANSI を篩って 4 つの数を出すだけ（30 行程度）。**単体テストは `git diff --no-index` で合成ディレクトリ 2 つを比較**する（repo の状態に依存しない・本日の実験がそのままフィクスチャになる）
5. 「上限どおり 40 束」は採らない。**根拠**: §5.1 の趣旨（読む量）に照らして、機械が保証した移動行はレビュアーが読まない行である。読まない行で束を割っても精度は上がらず、束の締めごとに払うレビュー費用（`/simplify` + チーム + Fable 監査）だけが 40 倍になる
