...
4. ~~🔴 **分割の束の数え方**（§13）~~ → 🔴 **裁定済み（owner 2026-09-12）: 採用。**
   `BUNDLE_BRANCH_WORKFLOW.md` **§5.1a** として制定した（residual で 1,500 判定 +
   ゲート (i) `moved+ == moved−` (ii) 短い moved ブロックは residual 扱い + 最終防波堤は裁定 6）。
   owner の言: **「フルレビュー 40 回はちょっと作業として重すぎる」**。見込みは子 1〜3 で **8〜9 束**
5. 🔴 **（新規・未裁定）出荷物ゲートの類型免除**: 「`git diff main...HEAD --name-only` が
   `tests/` `docs/` `*.md` に閉じる PR は出荷物ゲート（実機 E2E / cold install / cargo 3 行）を省く。
   判定はこのコマンドの出力が空であること」を規則にしてよいか。
   PR #894 は出荷物が main と **byte 一致**でゲートの情報量がゼロだったが、
   「条件分岐を付けない」規則を PR 単位で曲げないため全部回した（Fable と合意）。
   子 0b（設定と抑止コメントだけ）や dev サイトの PR でも同じ問いが出る
...
### 13.4 決定（🔴 **owner 裁定済み・2026-09-12**）

> **owner の裁定**: 「フルレビュー 40 回はちょっと作業として重すぎる」→ **residual + 3 重ゲートを採用**。
> `BUNDLE_BRANCH_WORKFLOW.md` **§5.1a** として制定済み。以下はその根拠と細目。
...
## §14 子モジュールの module doc の書き方（レビュー指摘を受けて確定・2026-09-12）

分割で起きる変更は 2 種類あり、**分けて述べる**。

1. **本文**（式・文・制御フロー・属性・doc コメント）— ここは 1 行も変えない。
   これが「既存テストの期待値が 1 つも変わらない」ことの根拠である
2. **可視性** — 分割の**必然**である。同一モジュール内では修飾子が要らなかった項目・
   フィールドが、親・兄弟から名前で参照されるために `pub(super)` を要する（設計 §5 の **E3′**）

### 🔴 件数を書かない

「**N 行を除いて純粋な移動である**」という書き方を禁じる。件数は後続の束で変わるのに doc は
追随せず、**宣言と実体がずれる面を作るだけ**だからである。`/code:pr-review-team` の
comment-analyzer が実測した実害:

| ファイル | doc の主張 | 実際 |
|---|---|---|
| `build_and_load.rs` | 「唯一の変更は `build` の可視性」 | `load_outproc_plugin_impl` も private → `pub(super)` の **2 件** |
| `startup.rs` | 「2 行を除いて」＋ 2 関数を名指し | 名指しの 2 関数は**このファイルに無い**（`startup_instrument.rs` にある） |
| `effect_slot_types.rs` | 「本文は 1 行も書き換えていない」 | `pub(super)` が **75 箇所**（本文は確かに不変だが、doc は可視性に触れていない） |

`startup.rs` / `startup_instrument.rs` は**説明がまるごと入れ替わって**いた。
2 段目の分割をしたとき、縮んだ側の doc を直さなかったためで、
§13.10 の「予告どおり踏んだ」と同じ**分割が成功した瞬間に実害化する**構造をしている。

### 書き方

- 可視性を **1 箇所も変えていない**なら、そう書く。`grep -c 'pub(super)' <file>` が 0 で検証できる
- 変えたなら「**本文は 1 行も書き換えていない。変えたのは可視性だけ**」と書き、
  **非自明なものだけ**を名指しで説明する（親のインラインテストから呼ばれる・兄弟から呼ばれる、等）。
  名指しするなら、**そのファイルに実在することを確かめてから書く**
