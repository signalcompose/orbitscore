...
| **ライブ切替で名前不一致** | **縮退しない。`AUDIO_DEVICE_UNAVAILABLE` を返し、鳴っているデバイスをそのまま使い続ける** | **owner 2026-09-05**（実装が上の裁定文と食い違っていた・Fable 監査 F4）。演奏中のタイプミスで内蔵スピーカーへ音が移るのを避ける。**起動時の縮退は据え置き** |
