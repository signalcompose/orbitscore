...
### 13.5.1 収束の単位を 2 つに分ける

4 件は**性質で 2 つに分かれる**。混ぜると順序の根拠が消えるので束を分ける。

| 束 | 何を収束させるか | 収束条件（これが満たされたら閉じる） |
|---|---|---|
| **束 A `777-line-router`** | 「chunk 列 → 行」の実装が **4 つ**あり、教訓が片方にしか適用されていない | 実装が **1 本の共有プリミティブ**に畳まれ、`extension.ts:311` のリングプロキシもそれを使う |
| **束 B `779-gated-environment`** | 実機の測定環境が信用できない（漏れ・許容値） | gated 全件を **3 回連続**で回して**失敗集合が一致**する（＝測定が再現する） |
...
```
#779（shm の漏れを止める）→ #775（U2 の許容を導出する）
```
...
**束 B に含める 3 つ目**（#780・2026-09-06 追加）: `c18_real_gain_obeys_the_decibel_contract`
...
**束 B に含める 4 つ目**: `orbitstudio-mcp-gated.spec.ts` の `:1396` / `:1589` / `:1615` の
...
**両束とも段 0（安全網）の残り**とする。§4.G が段 0 の結果を
「以降のすべての変更が退行を機械で検出できる」と定めており、**測定環境が信用できないうちは
それが成立しない**ため。段 2 以降と並行してよいが、**段 0 の完了条件からは外さない**。
