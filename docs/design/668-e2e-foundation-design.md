...
**当初は 2 束にしたが、「段 0 が完全に収束するまで先へ進めないのか」という owner の問いで
設計上の誤りが 1 つ見つかった。**

🔴 **「段 0 の完了条件」と「段 2 に進む前提条件」を同一視していた。** 段 0 が守るのは
...
さらに、4 件は**段 2 を止める性質が違う**:

| # | 段 2 を止めるか | 理由 |
|---|---|---|
| **#780** | 🔴 **止める** | **無条件マージゲート**なので、段 2 の**どの PR でも毎回**落ちて切り分けを強いる。「慣れて無視する」に至るとゲートとして死ぬ |
...
| 束 | 何を収束させるか | 収束条件 |
|---|---|---|
| **E-gate `780-merge-gate`** | 🔴 **段 2 に進む前提**。無条件マージゲートが信用できない | **#780 が 10 回連続で緑** + #779 の漏れが止まっている（実行前後で shm が増えない） |
...
🔴 **E-noise は段 2 と並行してよい。** #775 は **#779 / #780 を直した後でないと必要かどうかも
分からない**（環境の汚れが寄与要因の可能性）。**先に E-gate を入れて段 2 の実機で観測し、
...
### 13.5.3 束 E-gate — 段 2 に進む前提（#779 → #780 → 厳密等価 3 箇所）

> ✅ **完了（2026-09-07）**。PR [#789](https://github.com/signalcompose/orbitscore/pull/789) が main へ
> （merge commit `900d4532`）。**#779 / #780 / #785 は CLOSED**。段 2 の着手条件は満たされた。
>
...
### 13.5.4 段への配置（2026-09-06 夕・owner 裁定で改訂）

🔴 **当初の「両束とも段 0 の完了条件から外さない」は撤回する。** §13.5.1 のとおり、
**段 0 の目的（退行を機械で検出できる）は既に達成されている**。E-env が直すのは
測定のノイズであって、目的そのものではない。

| 束 | 段への配置 | 段 2 との関係 |
|---|---|---|
| **E-gate** | 🔴 **段 0 の完了条件に残す** | **段 2 に着手する前提**。無条件マージゲートが段 2 の全 PR に毎回コストを課すため |
| **E-router** | 段 0 の完了条件に残す | **並行可**。段 2 を止めない |
| **E-noise** | 🔴 **段 0 の完了条件から外す** | **並行可。段 2 の実機で「U2 が消えたか」を観測してから必要性を判断する** |

**着手のゲートと束の完了条件を分ける。**

| | 条件 |
|---|---|
| **段 2 に着手してよい条件** | **E-gate が閉じている**（#780 が 10 回連続で緑・#779 の漏れが止まっている） |
...
段 2〜8 が残っている規模に対し、測定のノイズをゼロにしてから進むのは投資として重すぎる。
