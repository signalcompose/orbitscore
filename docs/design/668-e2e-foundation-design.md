...
🔴 **ステージ 2 が「分割」の本命**。ファイルを割る（ステージ 4）のではなく**中身を割る**。
...
## 13.5 測定環境そのものが測定を汚す（#779 / #775 / #777 / #773・2026-09-06 追加）

束「gated の測定器」（#760 / #761 / #756）の作業中に、**測定器の外側**——測定が走る環境と、
測定器が依存する下位層——にも同じ「黙って測れなくなる」形が見つかった。本設計の §11（#624
孤児 daemon）・§12（#640 負荷で落ちるテスト）と同じ系列なのでここに続ける。

### 13.5.1 収束の単位を 3 つに分ける（2026-09-06 夕・owner 裁定で再編）

**当初は 2 束にしたが、「ステージ 0 が完全に収束するまで先へ進めないのか」という owner の問いで
設計上の誤りが 1 つ見つかった。**

🔴 **「ステージ 0 の完了条件」と「ステージ 2 に進む前提条件」を同一視していた。** ステージ 0 が守るのは
「**退行を機械で検出できる**」ことである。現在の実機は 27/29 が緑で、**残る 2 件は原因も帰属も
特定済み**——つまり**退行は既に検出できている**（新しい失敗が出れば既知の 2 件と区別がつく）。
E-env が直すのは「**測定のノイズを減らす**」ことであって、ステージ 0 の目的そのものではない。
**目的の達成と品質改善を混同していた。**

さらに、4 件は**ステージ 2 を止める性質が違う**:

| # | ステージ 2 を止めるか | 理由 |
|---|---|---|
| **#780** | 🔴 **止める** | **無条件マージゲート**なので、ステージ 2 の**どの PR でも毎回**落ちて切り分けを強いる。「慣れて無視する」に至るとゲートとして死ぬ |
| #779 | 止めない | ディスク・inode の圧力。**掃除で回避できる** |
| #775 | 止めない | 失敗が 1 件・場所が動くだけ。**既知として扱える** |
| 厳密等価 3 箇所 | 止めない | 偽赤/偽緑のリスクは残るが対象が限定的 |

したがって束を 3 つに分ける。

| 束 | 何を収束させるか | 収束条件 |
|---|---|---|
| **E-gate `780-merge-gate`** | 🔴 **ステージ 2 に進む前提**。無条件マージゲートが信用できない | **#780 が 10 回連続で緑** + #779 の漏れが止まっている（実行前後で shm が増えない） |
| **E-router `777-line-router`** | 「chunk 列 → 行」の実装が **4 つ**あり、教訓が片方にしか適用されていない | 実装が **1 本の共有プリミティブ**に畳まれ、`extension.ts:311` のリングプロキシもそれを使う |
| **E-noise `775-capture-clock`** | 測定のノイズ（U2 の許容） | gated 全件を **3 回連続**で回して**失敗集合が一致**する |

🔴 **E-noise はステージ 2 と並行してよい。** #775 は **#779 / #780 を直した後でないと必要かどうかも
分からない**（環境の汚れが寄与要因の可能性）。**先に E-gate を入れてステージ 2 の実機で観測し、
U2 が消えていれば #775 は不要になりうる。** 3 回連続の実測（1 回 15 分 + 負荷待ち）は
**この束で最も重い投資**なので、必要性が確定してから払う。

### 13.5.2 束 E-router — 行ルータの一本化

```
#777（engine 側に flush を足す）→ #773（stdout も部分行を持ち越す）→ ring proxy（未起票・本設計で拾う）
```

現状の 4 実装:

| 実装 | 場所 | partial 持ち越し | flush |
|---|---|---|---|
| `createLinePrefixer` | `packages/vscode-extension/src/extension.ts` | ✅ | ✅（#756 で入れた） |
| `createDaemonStderrLineRouter` | `packages/engine/src/audio/rust-engine/daemon-client.ts` | ✅ | ❌ **#777** |
| `setupStdoutHandler` の inline split | `extension.ts` | ❌ **#773** | — |
| 出力チャンネルのリングプロキシ | `extension.ts:311` | ❌ **未起票** | — |

🔴 **#757（requestId 相関ブリッジ 5 本の重複）の直前に置く。** #757 の共通化はこの層の上に
乗るので、逆順にすると 5 本を直した後でもう一度触ることになる。

**なぜ束にするか**: 4 箇所とも同じ不変条件（chunk 境界は行境界と一致しない／プロセスは行の
途中で死ぬ）を持つ。1 つずつ直すと、次の人がまた 1 つだけ直す。#756 の doc コメント自身が
「双子はこれを持たない」と書いていたのに双子を直さなかったのが、まさにその実例。

### 13.5.3 束 E-gate — ステージ 2 に進む前提（#779 → #780 → 厳密等価 3 箇所）

> ✅ **完了（2026-09-07）**。PR [#789](https://github.com/signalcompose/orbitscore/pull/789) が main へ
> （merge commit `900d4532`）。**#779 / #780 / #785 は CLOSED**。ステージ 2 の着手条件は満たされた。
>
> **実測**: #780 は **10 回連続で緑**（修正前は並列 5 回で 1 FAIL）／`orbit-outproc-*` は **957 → 25** で
> gated 全件の前後で増えない／実機 gated **29 passed / 1 failed**（唯一の失敗は main baseline の
> `steps the live playhead`・**新しい失敗ゼロ**）。
>
> 🔴 **対象は「厳密等価 3 箇所」ではなく 6 箇所だった。** 設計が挙げた 3 箇所に加え、`:1378` の
> `.toBe(0)`（**崩れる向きが逆＝偽緑**）と、ローカルラッパー `countAttachFailures` 越しの 2 箇所。
> 後者は**3 本のラチェットすべてに構造的に見えなかった**ため、検出器を「形の列挙」から
> **「log 由来の値を受けて件数を返す関数の解決」へ一段抽象化**した（不動点まで反復）。
>
> 🔴 **レビューで見つかった、この束自身の穴**: #780 の回帰テストが**どの自動経路でも走っていなかった**
> （`-- --ignored` は `#[ignore]` を付けたテストしか実行しない。実測 `0 passed; 19 filtered out`）。
> CLAUDE.md のマージ前ゲートに `--ignored` 無しの行を足した。
...
#779（shm の漏れ・1 実行で 957 個）は**環境の圧力**を作る。#780 の切り分けを
汚さないよう先に止める。**ただし #780 の原因は #779 とは独立**であることが上記で判明したので、
**並行してもよい**（順序の根拠は「切り分けの清潔さ」であって依存ではない）。
...
「値が本当に厳しすぎたから」なのか永久に分からない。

🔴 **#775 では閾値を先に緩めない。** `CLOCK_WALL_TOLERANCE_SEC = 0.12` は根拠コメントの無い
裸の定数（2048 frames @48kHz = 42.67 ms の約 2.8 バッファ相当）。緩めるなら
...
今回の AST ラチェットは「`Before` を含む識別子が算術に参加する」形だけを見るので対象外。
1 本目のラチェット（`bareErrorCountEqualityOffenders`）の守備範囲を広げる作業になる。

### 13.5.4 ステージへの配置（2026-09-06 夕・owner 裁定で改訂）

🔴 **当初の「両束ともステージ 0 の完了条件から外さない」は撤回する。** §13.5.1 のとおり、
**ステージ 0 の目的（退行を機械で検出できる）は既に達成されている**。E-env が直すのは
測定のノイズであって、目的そのものではない。

| 束 | ステージへの配置 | ステージ 2 との関係 |
|---|---|---|
| **E-gate** | 🔴 **ステージ 0 の完了条件に残す** | **ステージ 2 に着手する前提**。無条件マージゲートがステージ 2 の全 PR に毎回コストを課すため |
| **E-router** | 🔴 **2026-09-07 に再改訂 — 束として持たず割る**。**#773 はステージ 2 の PR に引き込む**（PR-O4 の直前）／**#777 は backlog**（完了条件にしない） | #773 は結合している: **PR-O4 が `//#evalBegin` / `//#evalEnd` を導入し、#773 が落とすのは `{"evalMark"` を含む封筒**。#777（daemon 死亡時の最終行）はステージ 2 の受け入れ基準に現れない |
| **E-noise** | 🔴 **ステージ 0 の完了条件から外す** | **並行可。ステージ 2 の実機で「U2 が消えたか」を観測してから必要性を判断する** |

> 🔴 **2026-09-07 追記（owner 指示）**: 「**ステージ 0、1 を完璧を目指してずるずるとやると
> 機能実装に進まない**」。§13.5.4 の「並行可」は**まだ全部やる前提**を残していたので、
> 計画 §3 で**引き込み条件**へ置き換えた — **そのステージの受け入れ基準が依存しているものだけを、
> そのステージの PR の中で直す。** 依存していないものは backlog に置き、完了条件にしない。
> 未カバー語はラチェットが増加を止めているので、**債務の上限は既に機械が押さえている。**

**着手のゲートと束の完了条件を分ける。**

| | 条件 |
|---|---|
| **ステージ 2 に着手してよい条件** | **E-gate が閉じている**（#780 が 10 回連続で緑・#779 の漏れが止まっている） |
| **E-noise を閉じる条件** | gated 全件を 3 回連続で回して失敗集合が一致（束の完了条件としては据え置き） |

**根拠**: 3 回連続の実測は 1 回 15 分 + 負荷待ちで、この系列で**最も重い投資**。
#775 は #779 / #780 を直すと消える可能性があるので、**必要性が確定してから払う**。
ステージ 2〜8 が残っている規模に対し、測定のノイズをゼロにしてから進むのは投資として重すぎる。

**残る 2 件の失敗は台帳に載せ、「新しい赤だけを見る」運用にする**
（`steps the live playhead` = main baseline / `#611 O0-4` = #775）。
...
| ステージ 2（本体をモジュールへ）で起動回数が増えない | **中** | vitest の発見単位はファイル。`it(` の登録が 1 ファイルに残る限り 1 セッション。反証: import 側の副作用でモジュールが二重評価される場合 |
...
| daemon 本数 `<= 1` が偽陽性を出さない | **中** | 別プロジェクト・別セッションの daemon が同時に走らない前提（`killOrbitStudio` と同じ専有前提）。反証: owner が手元で OrbitScore を開いたまま E2E を回す運用 |

---

## 22. 🔴 owner 裁定待ち（本文はこれに依存せず着手できる）

> **2026-09-03 owner 回答（裁定シート Q-668-1〜4）— 4 件すべて解消**
> - D-1 **C**（load average の報告を先に）/ D-2 **C**（観測を先に）/ D-3 **A 手動のまま**（owner 補足: スピーカーを絞っておけば無人で鳴らしてよい・近所の苦情は無い → 将来 B に切り替える障害は無い）/ D-4 **A + loud な注記**
...
| **D-4** | **`ORBIT_GATED_ONLY` を入れるか**（§7 ステージ 3） | **A**: 入れる（反復が速い）／**B**: 入れない（絞りっぱなしで全件が回らなくなる事故を避ける） | **A** + 「絞ったら結果に loud な注記」 | gated spec のみ |
