...
## 5b. 🔴 追補: プラグイン窓を OrbitScore の前面に出す（#940・owner 裁定 2026-09-14 で本 PR に畳む）
...
>
> 本節は当初「窓の重なり順は自動で観測できない」として**全部を手動ゲートに置いた**。**それは誤り**である。
>
> `CGWindowListCopyWindowInfo` の **`kCGWindowLayer`** が読める（0 = Normal / 3 = Floating）。
> しかもこれは**窓サーバ側の記録**なので、child に `window.level()` を聞く自己申告と違い、
> **実際に適用されたレベル**が取れる。
>
> 実測（同じ child・同じビルド・前面アプリだけを変えた）:
>
> | 前面のアプリ | layer |
> |---|---|
...
>
> → `tests/e2e/helpers/window-layer.{swift,ts}` と gated E2E `#940` を足した。
> 🔴 **区別するアサーションは「別アプリ前面で normal に戻る」**。常時 floating を掛ける実装でも
> 「ホスト前面で floating」は通ってしまう。
>
> **手動ゲートに残すのは「人が見て自然か」だけ**で、真偽の判定は E2E が持つ。
