...
> 🔴 **束への割り当て（owner 2026-09-07・#799）。正本は計画 §2.5。**
>
> | 束 | 統合ブランチ | 中身 | 検証 |
> |---|---|---|---|
...
>
> **PR-O3 を単独の束にする理由は分量ではない。** 本 PR は「旧 `SetBusRouting` の内部を program
> 生成へ写す（**互換維持**）」＝**振る舞いを変えない**ので、検証を
> 「**§10 の goldens が動かないこと**」で行える。**PR-O4 と同じ束に入れるとこの検算は永久に失われる**
...
## 14. ✅ owner 裁定（2026-09-03 に **8 件すべて解消**・PR-O4 は着手可能）

> 🔴 **見出しは 2026-09-07 まで「裁定待ち」のままだった。** 節末は「8 件すべて解消」と書いており、
> **本文と見出しが食い違いを起こしていた。** これが `/goal` の「§7.3 / §10.1 は裁定待ち」を
> 4 セッション延命させた一因。**節の状態は見出しに出す。**
