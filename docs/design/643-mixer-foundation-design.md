...
> この**土台がしっかりすれば**、OrbitScore の場合 **UI という制限がない**ので、より**柔軟だったり危険だったりするようなワイヤリングを表現できる**ようになるはずなんですよ。
...
> ✅ **埋まった（2026-09-03・#611）**: [`611-output-line-design.md`](611-output-line-design.md) が
> 出口を**ラインの 1 要素** `output(dest, thru:, db:)` として一般化した。宛先は
> master / sum / aux / render / 物理 ch 対 / **LinkAudio channel** が同じ軸に載り、
> ここで問うた `Link` も出口の一種として `OutputDest { kind: 'link' }` に収まっている
> （611 §2.2 / §5.3）。**入口（`(instance, unit)` アドレス）と出口の対称性はこれで揃う。**
>
> 🔴 本設計の `SourceDest { Master, Bus, Link }`（source → 土台への feed 宛先）は**そのまま残る**。
> 611 が一般化したのは**バス上のラインの出口**であり、層が違う（611 §5.6）。
...
> **2026-09-03 追記（#611 / owner 裁定）**: この 3 分岐という形自体が解消された。
>
> - **数値 render bus `output(n)` は撤回**（611 §14 (1) = A）。stem の宛先は `mix.render(...)` で
>   宣言したノードであり、`output(3)` は**物理アウト 3 番の mono 宛て**になる（core spec MX.2.3）
> - 残る分岐は「**宛先の解決**」1 本に統合される。`output` は kind で分岐せず、
>   予約語 → 宣言済みノード → 物理 ch 対 → LinkAudio の**解決順**だけを持つ（core spec MX.2.1）
> - したがって「midi の 2 分岐をエラーにするか」は、**受け手 × メソッドの適用可否表**
>   （[`610-diagnostics-applicability-design.md`](610-diagnostics-applicability-design.md) §3）
>   の 1 行として一元的に扱う。本節で個別に決める問いではなくなった
