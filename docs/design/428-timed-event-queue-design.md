...
> **2026-09-03 owner 回答（裁定シート Q-428-1〜4）— 4 件すべて解消**
> - (1) **B**（`play()` と同じ位置表記に param を置く）を v1。C（ハンドル）は #460 と同時
> - (2) **A 残す**。理由（owner）: Rust engine を単体で公開するので、汎用 wire は残す
> - (3) **A**（#460 の前提として起こす）。owner の懸念「再発明にならないか」→ ならない: サンプル精度の param イベントは CLAP（`clap_event_param_value` の `header.time`）/ VST3（`IParamValueQueue` の sampleOffset）が規格として持つもので、本 PR の仕事はそれを rack child まで運ぶ配線だけ（§3.6）
> - (4) **A 内部定数**。ユーザーには触らせない
