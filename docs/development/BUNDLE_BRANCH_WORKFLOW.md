...
| 語 | 意味 |
|---|---|
| 束（bundle） | まとめてレビューするのが合理的な PR の集合。**1 設計文書 = 1 束とは限らない** — 「出口の一般化」（設計文書 611・PR-O3〜O6）は **3 束**に分かれた（§10・2026-09-07・#799）|
...
main ──●────────────────────────────────────────────●──▶
        \                                          / 束 PR（merge commit 1 回・フルレビュー・実機全件）
         <issue>-<英語>（統合ブランチ）●──●──●──●──●
                                     ↑  ↑  ↑  ↑  ↑
                          小 PR（軽いゲート）: 1  2  3  4  fix
```

> 図の小 PR は本数の例。**1 束に何本入るかは束ごとに違う** — OrbitScore の実際の割り当ては §10。
...
| **束を開く前** | 🔴 **束が触る issue 番号を地図・計画・設計から `grep` し、記述が現状と合っているか確認する**（#814）| `grep -n '#NNN' docs/planning/*.md docs/design/*.md` |
...
| **束を閉じる前** | 🔴 **束で判明した事実（実測・分割・未検証範囲）が 3 文書に入っているか確認する**（#814）| `npm test -- planning-issue-state` |

## 5. 規則

### 5.1 束の大きさ — 差分 1,500 行以下で切る

束 PR の差分が大きいほどレビューの精度が落ちる。**束は最初から 1,500 行以下で切る**。

> 🔴 **数え方は変更行（`+` と `-` の合計）**（2026-09-07 明記・#799。正本は [`IMPLEMENTATION_PLAN_2026-09.md`](../planning/IMPLEMENTATION_PLAN_2026-09.md) §2.5）。差引き（net）で数えると同じ束が上限を跨いだり跨がなかったりする。**レビューが読む量は変更行**なので、上限の判定もそちらに揃える。
「途中でチェックポイント」を置くより、設計上の継ぎ目（wire と DSL、記録と再生、実時間とオフライン）で束を分ける方が単純で、継ぎ目は plan の並び順と一致する。旧 wire を併存させる方針なら wire だけの束でも main に入れて壊れない。

一方通行の決定（wire 形式・DSL 表面・ファイル形式）を含む PR は束の先頭か単独に置く。戻せない決定が監査を受けるのは遅くともその束の締めになる。

### 5.1a 🔴 分割（純粋な移動）の束は residual で数える（owner 裁定 2026-09-12・#888）

**分割の束は、`scripts/repo/move-residual.sh <base>...<head>` が出す residual 行数で
1,500 を判定する。素の変更行は記録するだけ。**

**なぜ変えたか**: §5.1 の上限の目的は「差分が大きいほど**レビューの精度が落ちる**」ことへの対処で、
単位は「**レビュアーが読む行**」である。純粋な移動でレビューが問うのは「このロジックは正しいか」ではなく
「**本当に純粋な移動か**」で、それは機械が答えられる。したがって residual で数えるのは上限の
**緩和ではなく趣旨どおりの適用**である。

素の変更行で数えると #888 の子 1〜3 だけで **約 40 束**になり、各束にフル編成のレビューが付く。
**機械が保証した移動行はレビュアーが読まない行**なので、読まない行で束を割っても精度は上がらず、
束の締めごとのレビュー費用だけが 40 倍になる（owner 裁定「フルレビュー 40 回は作業として重すぎる」）。

🔴 **ただし residual だけでは足りない。次の 2 つを同時に満たすこと**:

| ゲート | 何を捕まえるか |
|---|---|
| **(i) `moved+ == moved−`** | **複製**（消して 2 回足す）と**移動先の作り忘れ**（行が消滅） |
| **(ii) K 行未満の短い moved ブロックは residual として数える** | **1 文を関数間で移した変更**（振る舞いが変わるのに residual 0 になる） |

**(ii) が要る理由**: git の `--color-moved=zebra` は「**20 英数字以上**」なら **1 行でも**移動ブロックと
認めるので、「意味のある 1 文」がちょうど移動として吸われる大きさになる。K は #888 子 1 の最初の抽出で決める。

🔴 **最終防波堤は「既存テストの期待値を 1 つも変えていない」。** 機械的な residual 判定は
**レビューの入口を絞る道具**であって、意味論の保証ではない。振る舞いが変わればテストが落ちる
（落ちなければテストが弱いという別の問題）。

**実測の根拠**（2026-09-12・`docs/design/888-file-size-ratchet-design.md` §13.6〜§13.8）:

| 検証 | 結果 |
|---|---|
| 実素材（`engine_wrap.rs` から 362 行抽出） | 素の変更行 **686** → **residual 2**（0.3%） |
| 敵対ケース A〜D（ブロック内の変更 / 削除 / 挿入 / 移動先の作り忘れ） | **4 件とも仕込みが residual に出る** |
| 抜け道 E（1 文を関数間で移動） | residual **0** → **(ii) で捕捉** |
| 抜け道 F（消して 2 回足す = 複製） | moved+ 60 ≠ moved− 30 → **(i) で捕捉** |

**束の切り方**は行数ではなく**ソースファイル単位**にする: 1 束 = 1 つの元ファイルからの抽出
（大きいファイルは複数束に割ってよいが、**2 つの元ファイルを 1 束に混ぜない** — 退行が出た時に
bisect しないと帰属できないため）。

**分割束のマージ前ゲート**（既存に加えて）: (a) residual を PR 本文に貼る、(b) **テストパスに
限定した residual が `use` / `import` 行だけ**であること、(c) `cargo test` / `npm test` / 実機 gated 全件。

🔴 **上限だけで切らない — 「検算の機会」でも切る**（2026-09-07・#799 で実際にこれで 1 束を 3 束にした）。**振る舞いを変えない PR**（互換維持の配線入れ替え等）は、検証を「**既存の golden が 1 つも動かないこと**」で行える。これを**振る舞いを変える PR と同じ束に入れると、その検算は永久に失われる** — 後者では golden が*正当に*動くので、「配線の入れ替えで出力が変わったか」を二度と問えない。同じ理屈で、**旧経路を撤去する PR は新表面が実機で確かめられた後の束に置く**（同じ束だと、赤が出たときに「新しい表面が誤り」と「消したものがまだ必要だった」を区別できない）。**分量より先にこの軸で切る。**

🔴 **`tests/repo/file-size-baseline.json` の diff に `+` 行があれば、それは「値が増えた」か「エントリが増えた」のどちらかで、どちらもレビューで止める**（#888 子 0）。ファイル単位のコード行 500 超をラチェットで固定する仕組みで、baseline は現寸で登録され増やす編集は red・減らす編集は緑になるが、**値をソースと一緒に増やす編集**（ファイルを太らせて baseline も同じだけ上げる）は既存の他のラチェット（`worklog-size.spec.ts` 等）と同様に仕組みでは止まらない。レビュアーは baseline の diff だけ見ればよい。

### 5.1b 🔴 文書はいつ更新するか — 3 つの時点に分ける（#814）

**「後でまとめて書く」は必ず忘れる。** 2026-09-08 に実測した失敗はすべて
「**変わった直後に書かなかった**」ことが原因だった。

| 時点 | 何を | なぜ |
|---|---|---|
| 🔴 **事実が変わった瞬間** | **地図**（`DEVELOPMENT_MAP.md`）| 地図は「**今どうなっているか**」の層で、**次のセッションが最初に読む**。古いと申し送りが誤りを再生産する |
| 🔴 **決めた瞬間（実装の前）** | **計画・設計** | `CLAUDE.md` 運用規則 6「実装が仕様から逸脱する必要が生じたら **spec 側を先に更新**してから実装する（spec が正本）」|
| **束を閉じる前** | 3 文書の突合 | 次の束が読むものを揃える。上 2 つができていれば**確認だけ**で済む |

**実例（2026-09-08・束 O-wire）**:

| 何が変わった | いつ書くべきだった | 実際 |
|---|---|---|
| #801 が「負荷依存ではなく**スレッド数依存**」と判明 | 実測した直後 | 束の締めまで地図が古いまま。**旧 `/goal` の誤った前提はここから来ていた** |
| PR-O3 を O3a / O3b に割った | **割ると決めた直後**（実装の前）| 実装後に書いたので、設計監査に「束の中身が正本とずれる」と指摘された |
| 実機未検証の範囲が判明 | 実機を回した直後 | owner に問われるまでどこにも無かった |

### 🔴 「実測」を書くときは出典を必須にする

`tests/docs/planning-issue-state.spec.ts` が捕まえるのは**状態語の矛盾**（CLOSED なのに「未着手」）だけで、
**内容の誤り**（「#801 は負荷依存」— issue は OPEN のまま）は**捕まらない**。

そこは**運用で担保する**: **実測を書くときは日付か PR / commit を併記する。**
出典があれば、次の人が「これは 09-07 の測定で、09-08 に否定されている」と気づける。
**出どころの無い「実測」は、後から検証できない。**
...
2. **その PR が足した E2E だけを実機で**回す（対象を絞る仕組みを持つ。OrbitScore は suite 全体の on/off が `ORBIT_GATED_ORBITSTUDIO`、個々の絞り込みは vitest の `-t`。🔴 かつてここに書いていた `ORBIT_GATED_ONLY` は **doc 668 の決定 D-4 で「入れる」と確定しているが未実装**で、2026-09-06 に実測で判明した — 手引きが「既存の仕組み」として参照していたのが誤り。🔴 さらに **`-t` が効くのは自前でアプリを起動する自己完結テストだけ**で、suite 本体は先頭の 1 本が作った状態に依存するため絞ると落ちる。その場合は全件回すしかない）。委譲先の緑を根拠にしないための最低線。ここを省くと束の締めで一気に露見して fix が収束しない
...
🔴 **正本は [`IMPLEMENTATION_PLAN_2026-09.md`](../planning/IMPLEMENTATION_PLAN_2026-09.md) §2.5**。ここは写しなので、食い違ったら §2.5 が正しい。

| 束 | 統合ブランチ | 中身 | 概算 | 検証 |
|---|---|---|---|---|
| O-wire | `611-line-wire` | PR-O3 + #773 | 約 800 行 | **goldens が 1 つも動かないこと**（`OUTPUT_LINE_GOLDENS` / `O0-1`）+ cargo |
| O-surface | `611-output-line` | PR-O4 + daemon 台数のアサーション | 約 1,200 行 | E2E-2〜7 + E2E-10。🔴 **一方通行**（DSL 表面）|
| O-multiout | `611-multiout` | PR-O5・PR-O6 | 約 650 行 | E2E-9 + 全件緑 |
...
### 10.1 「出口の一般化」を 3 束に切った理由（2026-09-07・#799）

**分量は四番目**。詳細は [`IMPLEMENTATION_PLAN_2026-09.md`](../planning/IMPLEMENTATION_PLAN_2026-09.md) §3 が正本。

1. **O3 の検証は一度しか使えない機会** — PR-O3 は互換維持の配線入れ替え（**振る舞いを変えない**）なので、検証を「goldens が動かないこと」で行える。**O4 と同じ束に入れるとこの検算は永久に失われる**（O4 は DSL 表面を変えるので goldens は*正当に*動く）
2. **O6 は旧経路を消す＝逃げ道を塞ぐ** — O4 と同じ束だと、赤が出たときに「新しい DSL が誤り」と「消したものがまだ必要だった」を区別できない
3. **一方通行と可逆を混ぜない** — O4 は一方通行、O5 / O6 は戻せる
4. 分量 — O4+O5+O6 = 約 1,850 変更行で上限 1,500 を超える

**払うコスト**: 束の締めのフルレビューが 1 回 → 3 回（増えるのは締めのレビュー 2 回分。束の中の小 PR はどのみち分かれている）。
