...
**次（増分 5+ = producer↔consumer を実 render に繋ぐ）**: ① render_multi を cpal callback に配線（4 sample-format branch を `CpalCallbackState` に refactor・N RingTapSink を保持・try_lock 競合で全バッファゼロ・登録 command ring を冒頭 drain）② EngineWrap で ring 生成・GPL consumer thread spawn（`#[cfg(feature="link-audio")]`・RingTapSink→native callback / Consumer+drops→`LinkChannelEgress`）③ `RegisterLinkAudioChannel` daemon command ④ TS `registerLinkAudioChannel` 実配線。**dynamic registration の pool + readiness race は 2b-2b**（2b-2a は 1 channel 実証で race を隔離）。**注意（@claude/レビューで surface 予定）**: receiver shim は verification 専用で production egress(sender-only)を変えないが、production shim に LinkAudioSource がコンパイルされる点を明示。
