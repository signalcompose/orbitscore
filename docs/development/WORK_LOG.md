...
（`wav-analysis.ts:106`・0 か範囲外のときだけ EOF まで読む）。一方 `sync_header` は
**約 1 秒ごと**にしか patch しない。つまり区間は「`stat.size` 時間」で刻まれ、
...
`send` / `effect` を足すため発生する。録り幅を `小節 + 位相 + n·P + guard` の式に直した
（4800 → 6800 ms）。**snap は最初の onset から厳密に `n·P` を測るので golden の値は動かない。**
