...
**follow-up 3 件を #332 に畳む（owner 判断「先にやる」）**: ① **#2 scheduleEvent/scheduleSliceEvent の stale「not wired」warn を削除**（egress 配線済みで誤誘導・feature-gap は registerLinkAudioChannel が authoritative・`a82d037`。stopAll 再 arm test は masterEffect vehicle に切替）② **#1 VerificationReceiver lifetime を PhantomData で型強制**（host-outlives-receiver を compile-error 化・2b-2b で receiver call site 増＝now-relevant・両 gated path 緑・`a5614f8`）③ **#3 LinkEgressStats: ring-drops を 1Hz ticker で surface**（silent-failure が 2a+2b で挙げた・control が drop counter clone 保持→`total_ring_drops`→EngineWrap→session ticker が増加で `LINK_EGRESS_DROP` WARNING DaemonError event・`4d9dd44`）。**追加分を /simplify + pr-review-team で再収束完了（`7777c27` + `e916cdc`）**: /simplify は `registered: HashMap<String, Arc<AtomicU64>>`（name→drop counter）への統合を提案＝冪等判定 set と drop 集約を 1 map に畳み altitude の correlation 懸念も解消（`7777c27`）。pr-review-team は IMPORTANT 2 を反映 = ① **TS が `daemon-error` イベント未購読**（#3 の ring-drop が surface されても operator に届かず end-to-end が塞がる load-bearing）→ `rust-engine-player.ts` に `onDaemonError` 購読を establishSession で off→on / quit で off ② **`LINK_EGRESS_DROP` の integration test 欠如** → `protocol.rs` に `daemon_error_warning_on_link_egress_drop`（`link_egress_drops_arc().fetch_add` → 1Hz ticker が DaemonError WARNING emit を確認・StubBackend でも駆動できる test seam）。Minor（session.rs コメント整合）も反映（全て `e916cdc`）。検証（再実行で実証）: daemon 18 passed（新 integration test 含む）/ TS player spec 38 passed（新 subscriber test 含む）/ cargo-deny `licenses ok`（GPL 非漏洩）。
