...
#### 🔴 背景セキュリティレビューが初版の穴を 3 件指摘した

書いた直後に自動レビューが走り、**実在するバイパスを 3 件**出した。いずれも実証してから直した。

| 指摘 | 初版の問題 | 直し方 |
|---|---|---|
| parser differential | `gh pr merge --admin 744`（**フラグが先**）は「`merge` の直後の数字」を見る正規表現に当たらず**素通り**（実証: exit 0） | `merge` 以降のトークンを走査し**最初の裸の整数**を拾う |
| fail-open | `gh pr view` が失敗すると `exit 0` = **マージ許可** | **fail-closed** に反転。判定できなければブロック — **判定を壊せば必ず通るゲートは、ゲートとして機能しない** |
| matcher bypass | `Bash:gh pr merge.*` は**先頭一致**なので `cd x && gh pr merge …` に当たらない | matcher を `Bash:.*gh pr merge.*` に |

修正後、**全 7 パターン**を通しで再検証した（フラグ先行 / `cd` 前置 / `--squash` / docs のみ ×2 /
判定不能 / マーカー有り）。すべて期待どおり。

🔴 **「ゲートを書いた」だけでは足りない。ゲート自体が回避できないかを試す必要がある。**
今回それをやったのは自動レビューで、私は初版を「検証済み」として出していた。
