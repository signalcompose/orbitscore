...
### 6.333 test(e2e): 3点キャプチャ + ピッチオラクル + パス束縛。実機変異で aux の信号欠落を発見 (Jul 30, 2026)

**Date**: 2026-07-30
**Issue**: #577 (PR-A) / **PR**: #585 / **発見**: **#587**
**Status**: ✅ 全 1836 passed / 34 skipped / 0 failed・**実機 gated 6 passed**・**製品バグ1件を実測で発見**
...
#### 🔴 実機変異で製品バグを発見 — aux は音を運んでいない（#587）

各 receiver の insert の gain を既定へ落として録音の peak 変化を実測:

| 変異対象 | Fable の線形モデル | **実測** | 判定 |
|---|---|---|---|
| **master** | 44.4% | **44.4%**（0.44444） | ✅ 一致 |
| **sum** | 41.2% | **41.2%**（0.41176） | ✅ 一致 |
| **aux** | 23.1% | 🔴 **0%** | **寄与なし** |

**aux の gain を `0.0`（完全無音）にしても測定値が動かない。**
直列レグ `seq → sum → master` はモデルどおり小数点以下まで一致しており、
**死んでいるのは aux レグだけ**。→ **#587**
...
「**登記の内容一致 + 復元元パス束縛まで。音声接地は #587 の解決待ち**」と正確に書き直した。
