...
**次（増分 4+ = proven core を実 render に配線）**: render_multi を cpal callback に配線（RingTapSink port を native〔permissive〕へ + 4 sample-format branch を `CpalCallbackState` に refactor + try_lock 競合で全バッファゼロ）+ EngineWrap で ring 生成・consumer thread spawn（`#[cfg(feature="link-audio")]`・producer→native / consumer→GPL thread）+ `RegisterLinkAudioChannel` daemon command + TS `registerLinkAudioChannel` 実配線。**dynamic registration の pool + readiness race は 2b-2b**（2b-2a は 1 channel 実証で race を隔離）。**注意（@claude/レビューで surface 予定）**: receiver shim は verification 専用で production egress(sender-only)を変えないが、production shim に LinkAudioSource コードがコンパイルされる点はレビューで明示。
