...
**次（増分 6+ = daemon 配線で 1 channel end-to-end）**: ① EngineWrap（`#[cfg(feature="link-audio")]`）= `start_default_output_with_link_egress` で起動・GPL consumer thread を spawn（`LinkAudioOutput` + `LinkChannelEgress` を保持し pump ループ）・`register_link_audio_channel` で RingTapSink 生成→sink を reg-ring 経由 callback へ / consumer+drops を mpsc 経由 consumer thread へ。**consumer thread lifecycle を明示**（spawn を callback push より前に・teardown は signal+join で drop 順非依存＝A0 §13 教訓）② `RegisterLinkAudioChannel` daemon command（string-keyed match arm）③ TS `registerLinkAudioChannel` 実配線 ④ gated daemon-level 層B テスト（実 cpal stream→ring→consumer→Link→in-process receiver）= 2b-2a Done 基準。**dynamic registration の pool + readiness race は 2b-2b**（2b-2a は 1 channel 実証で race を隔離）。**注意（@claude/レビューで surface 予定）**: receiver shim は verification 専用で production egress(sender-only)を変えないが、production shim に LinkAudioSource がコンパイルされる点を明示。
