...
## 14. S2 実行結果 / Verdict（2026-06-20）— サンプル経路 seam 差し替え（parity proof）

S1/S1b の CLAP hosting とは**別軸**の S2 = **サンプル経路の音声ディスパッチ seam 差し替え**（master plan §4-A）。Issue #296 / PR（`296-daemon-dispatch-seam-parity`）。§13 末尾の「S2/A4 carry-forward」は CLAP hosting の daemon 統合段の話で、本 §14 のサンプル経路 parity とはスコープが異なる（CLAP の daemon 統合は S2 のスコープ外＝後続段）。
...
- seam = **バックエンドレベル**。`AudioEngineBackend`（`Scheduler` + AudioEngine 面）を新設し、`SuperColliderPlayer`（無改変）と新規 `RustEnginePlayer` がともに満たす。`InterpreterState.audioEngine` を具象型→interface 化、`createAudioEngine()` が env で分岐。
- **lean daemon scheduler**: SC EventScheduler（LinkAudio/bufnum/`/s_new` 結合が重い）は再利用せず、1ms poll を mirror した独立スケジューラ。cancellation（clear/mute/loop）が TS 側に残り既存意味論を保つ。
- **timing モデル = poll-and-fire-now + 定数 lookahead（50ms）**。SC=fire-now / daemon=schedule-ahead（自前 transport clock, boot で 0）を、poll 発火時に `playAt(daemonNowSec + lookahead)` で繋ぐ。**定数 lookahead が相対 timing を保存**しつつ daemon render cursor を上回らせ onset clip を避ける。clock anchor は `StreamStats`(1Hz) の transport now_sec（Rust: `engine.transport_or_uptime_sec()` — transport 未開始時は uptime_sec にフォールバック）で毎秒補正（audio/wall drift 吸収）。

### 計測結果（実 daemon・ground truth = observer 接続の StreamStats、kick.wav を 8 hit × 300ms）

`ORBIT_REAL_DAEMON=1` の代表的 single run（値は run ごとに数 ms 変動。gated テストは下記の**境界**を assert し、特定値は固定しない）:

| 指標 | 結果（複数 run の幅） | 判定 / assert 境界 |
|---|---|---|
| transport rate（Δnow_sec/Δwall） | **≈1.00**（0.998–1.004） | ≈1.0 = clock mapping 前提成立（assert 0.95–1.05） |
| dispatches | **8/8** | 全 hit 発火 |
| lead `time_sec − trueNow` | **min ≈38–48ms / max ≈48–58ms** | **全て正 → onset clip しない**（assert minLead > 0） |
| anchor drift `|est − truth|` | **max ≈3–12ms** | 推定が真値と lookahead 内（assert < 50ms） |
| inter-onset 誤差（vs 300ms） | **max ≈2–7ms** | **相対 timing 保存**（assert < 50ms） |
| 観測 xruns | **0** | 発音中グリッチ無し（assert == 0） |

**polymeter 実証**（同 gated spec の 2 つ目）: seqA=400ms / seqB=300ms（3:4）を同時走行し、各シーケンスの inter-onset 誤差 **≤7ms**・xruns 0 → 異なる周期が daemon 経路で**独立に保存**される（polymeter parity を by-construction でなく demonstrated に）。

### Verdict = **PASS**

poll-and-fire-now + 定数 lookahead で、SC の fire-now と daemon の schedule-ahead を parity-grade（単一/polymeter とも相対 timing 誤差 ≤7ms・onset clip 無し・xruns 0）で繋げることを実機で実証。Stop&Report 条件（ready/handshake 破綻・parity 不成立・seam が嵌まらず DSL 改変が必要）はいずれも非該当（seam は嵌まり、既存 SC 経路は無改変・1151 既存テスト無傷）。

**実証の境界（正直な明記）**: 計測は RustEnginePlayer を直接駆動した単一/polymeter パターンで行った。`.orbs` を full interpreter（`ORBITSCORE_ENGINE=rust`）に通した end-to-end は未実施。DSL→Sequence の周期計算は**不変の TS 層**（S2 で無改変・1151 unit でカバー）の責務であり、本計測は backend が与えられた時刻を崩さないことを示す。MIDI↔audio クロック同期（master plan §4-A の唯一の統合チェックポイント）は `RustEnginePlayer.startTime` を `EventScheduler` と同タイミングで打刻し `TransportClock` を無改変に保つことで **by-construction で維持**（end-to-end 実走は未実施）。

### feature gap（§6 確定で defer・boundary で明示）

`RustEnginePlayer` は見かけの parity を作らないよう public boundary で warn する: pan≠0 → 中央定位（pan drop）/ slice/chop → skip（A3 time-stretch・#239）/ outputChannel(LinkAudio) → hardware fallback（A4）。内部 `ScheduledPlay` は pan を保持（param-complete）し、後の pan 追加を配線だけにする。

### S2 後に残る後続

pan の daemon 実装（ScheduledSample + PlayAt + engine mixing の小改修）/ slice・time-stretch（A3）/ LinkAudio・master effects の Rust 隔離（A4）/ 出荷 .vsix の cutover（engine 成熟後）/ daemon 側 hard-stop-all（現状 in-flight voice は自然減衰）。
