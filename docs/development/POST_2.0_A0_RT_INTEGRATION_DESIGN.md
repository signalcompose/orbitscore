...
- **timing モデル = poll-and-fire-now + 定数 lookahead（50ms）**。SC=fire-now / daemon=schedule-ahead（自前 transport clock, boot で 0）を、poll 発火時に `playAt(daemonNowSec + lookahead)` で繋ぐ。**定数 lookahead が相対 timing を保存**しつつ daemon render cursor を上回らせ onset clip を避ける。clock anchor は `StreamStats`(1Hz) の transport now_sec（`engine.now_sec()`）で毎秒補正（audio/wall drift 吸収）。

### 計測結果（実 daemon・ground truth = observer 接続の StreamStats、kick.wav を 8 hit × 300ms）

| 指標 | 結果 | 判定 |
|---|---|---|
| transport rate（Δnow_sec/Δwall） | **1.0038** | ≈1.0 = clock mapping 前提成立 |
| dispatches | **8/8** | 全 hit 発火 |
| lead `time_sec − trueNow` | **min 47.6ms / max 57.8ms** | **全て正 → onset clip しない**（最小でも +47.6ms） |
| anchor drift `|est − truth|` | **max 7.8ms** | 推定が真値と 8ms 以内（lookahead 50ms が十分カバー） |
| inter-onset 誤差（vs 300ms） | **max 5.1ms** | **相対 timing 保存 → quantize/polymeter parity** |
| 観測 xruns | **0** | 発音中グリッチ無し |

### Verdict = **PASS**

poll-and-fire-now + 定数 lookahead で、SC の fire-now と daemon の schedule-ahead を parity-grade（相対 timing 誤差 ≤5ms・onset clip 無し・xruns 0）で繋げることを実機で実証。Stop&Report 条件（ready/handshake 破綻・parity 不成立・seam が嵌まらず DSL 改変が必要）はいずれも非該当（seam は嵌まり、既存 SC 経路は無改変・1129 既存テスト無傷）。
...
pan の daemon 実装（ScheduledSample + PlayAt + engine mixing の小改修）/ slice・time-stretch（A3）/ LinkAudio の Rust 隔離（A4）/ 出荷 .vsix の cutover（engine 成熟後）/ daemon 側 hard-stop-all（現状 in-flight voice は自然減衰）。
