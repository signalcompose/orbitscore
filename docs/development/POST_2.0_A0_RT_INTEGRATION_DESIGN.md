...
| 指標 | 結果（代表 run・変動幅） | 判定 / assert 境界 |
|---|---|---|
| transport rate（Δnow_sec/Δwall） | **≈1.00**（0.998–1.004） | ≈1.0 = clock mapping 前提成立（assert 0.95–1.05） |
| dispatches | **8/8** | 全 hit 発火 |
| lead `time_sec − trueNow` | **min ≈45ms / max ≈50ms**（複数 run で 44.8–57.8ms） | **全て正 → onset clip しない**（assert minLead > 0） |
| anchor drift `|est − truth|` | **max ≈5ms**（複数 run で 2.9–7.8ms） | 推定が真値と lookahead 内（assert < 50ms） |
| inter-onset 誤差（vs 300ms） | **max ≈3ms**（複数 run で 1.8–5.1ms） | **相対 timing 保存 → quantize/polymeter parity**（assert < 50ms） |
| 観測 xruns | **0** | 発音中グリッチ無し（assert == 0） |

### Verdict = **PASS**

poll-and-fire-now + 定数 lookahead で、SC の fire-now と daemon の schedule-ahead を parity-grade（相対 timing 誤差は複数 run で最大 5.1ms・onset clip 無し・xruns 0）で繋げることを実機で実証。Stop&Report 条件（ready/handshake 破綻・parity 不成立・seam が嵌まらず DSL 改変が必要）はいずれも非該当（seam は嵌まり、既存 SC 経路は無改変・1129 既存テスト無傷）。
