...
複数バッチで計測（各 buffer 6 回程度）。全 run で `measurement_valid: true`・`overruns: 0`。RT 安全の
判定軸である **worst-case callback_max** は全 run の最悪値を採る。

| buffer | block budget | rt_mean (典型) | rt_p99 (run 幅) | **callback_max (全 run worst)** | **worst / budget** | overruns | 判定 |
|-------:|-------------:|---------------:|----------------:|--------------------------------:|-------------------:|---------:|:-----|
| 512    | 11.61 ms     | 4〜21 µs        | 6 µs 〜 1.15 ms  | **2.15 ms**                     | **18.5 %**         | 0        | **PASS** |
| 128    | 2.90 ms      | 9〜16 µs        | 56 µs 〜 0.56 ms | **2.83 ms**                     | **97.6 %**         | 0        | 余裕ゼロ（reliably safe とは言えない） |
| 64     | 1.45 ms      | 9〜16 µs        | 0.1 〜 0.31 ms   | **4.15 ms**                     | **286 %**          | 0        | **違反** |
...
roundtrip_max_ns:      4903042   (= 4.9 ms)
child_respawns:        1
...
- **host プロセスは生存**（結果を出力＝親は落ちていない）→ C-ABI segfault がプロセス境界で
  **封じ込められた**。
- recovery が 4.9ms で完了し、影響を受けた callback すら budget(11.6ms)内 → **無音落ちすら無し**
  （`overruns: 0`）。
...
measurement_valid:     true
roundtrip_overruns:    46        ← glitch-to-silence(respawn 窓 ≈ 46×1.45ms ≈ 67ms)
callback_max_ns:       800709    ← timeout で頭打ち = deadlock しない
child_respawns:        1
recovered_after_crash: true
post_mix_peak:         0.25000
```
...
- 46 callbacks が glitch-to-silence（≈67ms = process respawn 窓: spawn + dlopen + mmap + attach。
  respawn のたびに変動し数十 ms オーダー）。この無音窓は `roundtrip_overruns × block 周期`で読む
  （誤解を招く派生メトリックは持たない）。
