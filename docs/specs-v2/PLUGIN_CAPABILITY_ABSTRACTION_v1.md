...
| 2. 音色を作る / 選ぶ | プラグイン UI（GUI つまみ） | **DSL に書くオートメーション** / preset 選択 | `CAP-UI-*` / `CAP-PARAM-*` / `CAP-PRESET-*` |
| 3. 記録 | **自動スナップショット**（GUI 操作は DSL に残らないため） | **不要** — 値は `.orbs` 自身が持つ | `CAP-STATE-GET` |
| 4. 復元 | 自動（共通） | DSL の再評価（+ 共通の自動復元） | `CAP-STATE-SET` |

### 🔴 2 本足の非対称（owner 確定・2026-07-30）

**段階 2 以降で、人間と LLM は「永続化される場所」が違う。**

| 操作 | 使う人 | DSL に残るか | スナップショット |
|---|---|---|---|
| **GUI のつまみ** | **人間のみ** | 残らない | **必要**（= 自動 state 記録 #577 の存在理由） |
| **オートメーション** | **人間 + LLM 共用** | **残る**（`.orbs`） | **不要 — DSL が担保する** |

- **オートメーション値を state に保存しない。** 保存すると `.orbs` と state の 2 箇所が
  同じ値の正本を主張することになる
- **LLM がパラメータを操作する第一級経路は DSL である。** MCP が param を直接設定する経路を
  第一級に据えない（DESIGN_PRINCIPLES §4「意図と登記の分離」のパラメータ層への適用）。
  LLM 面の DSL 表面は #506（`seq.PluginName(param: value, …)`）
- **人間の GUI 経路は代替不可。** 手書き state ファイルでの代替は owner 却下済み

**両面は同じプラグインインスタンスに合流する**（DESIGN_PRINCIPLES §3）。
...
7. **必須能力には観測面が対応して存在する**。`CAP-PARAM-*` / `CAP-PRESET-*` の
   **列挙・取得**（param の id / 名前 / 範囲 / 現在値、利用可能な preset）は MCP tool として
   提供され、UI の実装有無にかかわらず LLM から観測できる（DESIGN_PRINCIPLES §1）。
   tool 名・引数・観測形の詳細は実装 PR で定め、本仕様からは「存在すること」のみを要求する。
   **ただし確定後は本仕様へ反映する** — spec が単一信頼情報源であり、tool のスキーマが
   コードにしか無い状態を恒久化させない（DESIGN_PRINCIPLES §5）

   > 🔴 **設定（値の変更）は MCP を第一級経路にしない**（CAP.4 の 2 本足決着・2026-07-30）。
   > LLM がパラメータを変えるときは **DSL を書いて評価させる**（#506）。理由は
   > 「MCP 直接設定の値が `.orbs` に残らず、DSL とプラグイン state のどちらが正本かが
   > 決まらなくなる」ため。MCP に設定系 tool を置くこと自体は禁じないが、**それは計測・
   > デバッグ用の副経路**であり、CAP.4 のループはこれに依存してはならない。
   > `CAP-PARAM-SET` の能力そのもの（child がパラメータを設定できること）は、
   > DSL 経路の実装先として引き続き必須である
