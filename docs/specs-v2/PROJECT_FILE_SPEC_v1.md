...
#### トリガ (c) の証明の深さ（実機 gated E2E・変異検証で実測）

| receiver | 自動記録 | 復元 | **音声接地** |
|---|---|---|---|
| **master** | ✅ | ✅ | ✅ **実測 44.4%**（gain を既定へ落とすと assert が赤） |
| **sum** | ✅ | ✅ | ✅ **実測 41.2%** |
| **sequence effect** | ✅ | ✅ | ✅（直列レグ上・モデル 37.5%） |
| **instrument** | ✅ | ✅ | ✅ 基本周波数の一致（非既定の半音オフセット） |
| **aux** | ✅ | ✅（登記の内容一致 + 復元元パス束縛） | ✅ **RMS 許容 2% への締め付けで判別可**（欠落 4.11% > 2%・[#587](https://github.com/signalcompose/orbitscore/issues/587)） |

**aux の寄与は実在する**（send は信号を運んでいる・RMS で **−4.11%** 符号つき実測）。
当初「寄与ゼロ」に見えたのは信号経路ではなく**測定側**の問題だった（#587 診断で確定）:

- OOP insert はパイプライン化（+1 block 遅延）されており、**aux リターンは直行レグより
  1 device block 遅れて** master に届く。kick のファイル peak はオンセット後 ~1.4ms
  （その block 内）にあるため、**全体 peak は許容値によらず g_aux に厳密に不感**
  （数値検証で g_aux ∈ {0.95, 0.5, 0.0} すべて合成 peak 比 1.00000）
- 全体 RMS は **−4.11%** 動くが、旧許容 15% の帯域内に沈んでいた。実測ノイズ床
  （無変異・同一設定の2回 capture）は **3.4e-6** と桁違いに小さいため、
  **RMS 許容を 2% に締めて aux 欠落を red にした**（ノイズ床の ~6000 倍・最小故障の ~1/2）

aux の証明は次の**4点構成**で閉じる:

1. **欠落が音で red になること**（restore fidelity の RMS assert・許容 2%）
2. **登記されたバイト列が正しいこと**（決定論的ファイル名との完全一致 + 内容一致）
3. **復元時に正しいファイルが送られること**（restore ログの**パス束縛**）
4. **sum 発 send → aux insert → リターンが信号を運ぶこと**（daemon gated テスト
   `set_bus_routing_wires_sum_send_to_aux_and_return` のバス単位 closed-form 実測）

> **教訓**: #564 は aux を「復元ログ行で daemon 往復が通る」ことだけで証明していた。
> **ログは送信の証明であって適用の証明ではない** — 適用の証明はバス単位の実測（上記4）で
> 初めて閉じた。なお #587 は当初「aux は音を運んでいない」と誤判定された（peak だけを
> 根拠にしていた）。**1つの測定指標が不感であることを「信号の不在」と読み替えない**こと。
> 閾値つき assert の緑/赤で判定する前に、**その指標のノイズ床を測る**こと（15% は
> ノイズ床 3.4e-6 の 44000 倍で、判別力を捨てていた）。

**音色の起点は手書き state の登記**であり、受け入れ基準の「**UI で**音色を作る」要素は
**#474 の UI 実装後にのみ閉じられる**。

**(b) は #474（プラグイン UI の open）を前提とする。** UIH.4 の3経路は「開いた UI を
閉じる」経路なので、**UI を開く手段が存在しない現状では閉じる経路も存在しない**。
したがって PRJ.9 の「3つの閉じる経路それぞれでセーフポイントがちょうど1回発火する」
変異検証は **#474 の完了後に初めて実施可能**であり、それまでは未達を欠陥として扱わない。
...
### 🔴 ファイル名は衝突しない可逆エンコードで導く

登記キー（`kick/effect/reverb/0`）からファイル名を作る際、**セグメント区切りを単純な
`-` フラット化にしてはならない**。セグメント自体が `-` を含むと別インスタンス同士が同名に
なり、**黙って state を共有する**（例: `a-b/c` と `a/b-c` がどちらも `a-b-c` になる）。

**キー → ファイル名は衝突しない可逆エンコードであること**（区切り文字のエスケープ、または
キーのハッシュを付す等）。本文の例は可読性のために簡略化した表記であり、実装の規定ではない。
...
- **(b) UI クローズ時の「3つの閉じる経路それぞれで1回」は #474 完了後に実施する**
  （上記「実装状況と追跡先」を参照）
