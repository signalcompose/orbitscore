...
| (c) 停止・終了時 | **実装済み**。証明の深さは receiver 種別ごとに違う（下表） | #577 PR-A（UI 要素は #474 / aux の音は #587） |
...
| 🔴 **aux** | ✅ | ✅（登記の内容一致 + 復元元パス束縛） | 🔴 **不可能 — [#587](https://github.com/signalcompose/orbitscore/issues/587)** |

🔴 **aux の音声接地は原理的に取れない。** **aux バスの insert が出力音にまったく寄与していない**
（gain を `0.0` にしても録音の測定値が動かない）ことが実測で判明した（#587）。
これは記録・復元の欠陥ではなく **aux の信号経路そのものの問題**であり、
**#587 の解決までは aux の「同じ音で鳴る」を音で証明できない**。

aux について現在証明できているのは:
**登記されたバイト列が正しいこと**（決定論的ファイル名との完全一致 + 内容一致）と、
**復元時に正しいファイルが送られること**（restore ログの**パス束縛**）まで。

> **教訓**: #564 は aux を「復元ログ行で daemon 往復が通る」ことだけで証明していた。
> **ログは送信の証明であって適用の証明ではない** — 送信は証明されていたが、
> **信号自体が届いていなかった**。音で測って初めて出てきた。
