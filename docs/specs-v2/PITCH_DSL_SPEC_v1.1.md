...
2.  **Active note tracking**: シーケンスごとに発音中ノート(pitch, channel)を追跡。以下で確実に解放する:
    ...
    - **一発 `RUN()` が終端に達した時**: 当該シーケンスの保留ノートに note-off（#606）
    - **オフラインレンダが終端に達した時**: 同じ flush を呼ぶ（発火点だけが違い、flush 自体は同じ。#598 / #606）
    - `global.stop()` / エンジン終了 / クラッシュハンドラ: 全チャンネルに CC123 (All Notes Off) + CC120 (All Sound Off)

    🔴 **発火点が増えても配送機構は 1 本である**（#606）。上のどの場面でも呼ぶのは同じ
    「owner の保留ノートを解放する」経路であり、場面ごとに別の flush を作ってはならない。
