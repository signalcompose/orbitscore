...
🔴 **棚卸しの入口は自動で出る**（2026-09-08・#814）。`tests/docs/planning-issue-state.spec.ts` の
`KNOWN_STALE_BASELINE` は、**閉じた issue を未完了の語で書いたまま残っている行**の台帳である
（起案時点で 3 件 + 誤検知 3 件）。**棚卸しで直したら baseline から削る** — 直したのに残っていると、
次に同じ行が腐っても検出できない（それ自体を別の `it` が red にする）。
🔴 **baseline を増やして通さない。** 増やす編集は「閉じた issue を未完了として書いた」ことである。
...
| 小 PR を閉じる | CI 緑 + その PR が足した E2E を実機で + main が差分を読む → 統合ブランチへ merge | `gh pr merge <n> --merge` |
