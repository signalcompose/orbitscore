...
**再検討しない**（owner 明言・2026-07-29）。

**唯一の例外は「Apple が再び Intel を採用した場合」**。つまりこの決定が依存しているのは
**Rosetta の寿命ではなく、Apple が出荷する CPU アーキテクチャそのもの**である。
上記の Rosetta 廃止スケジュールは決定の**背景**であって条件ではない
（Apple が Rosetta の方針を変えても、Intel Mac 対応は行わない）。

Intel Mac 対応 / Rosetta ヘルパー / universal binary を提案・実装しないこと。
「念のため」「将来のために」も不要。
