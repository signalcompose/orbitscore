...
```typescript
// packages/vscode-extension/src/extension.ts:447-458
...
```typescript
// packages/vscode-extension/src/extension.ts:3072-3109
...
```typescript
// packages/vscode-extension/src/extension.ts:1505-1513
...
```typescript
// packages/vscode-extension/src/extension.ts:140-150
...
```typescript
// packages/vscode-extension/src/extension.ts:303-314
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:88-94
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:180-190
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:169-171
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:455-476
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:273-279
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:1060-1065
...
```typescript
// tests/e2e/helpers/engine-log.ts:80-91
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:1601-1615
...
The `#643` tests go one step further and compare RMS per time segment. Segment boundaries are read straight off the **byte length of the capture file** (`(stat.size - 44) / (channels x 4) / sampleRate`), and the first segment only opens **once sound has been detected**. The 20 ms-window RMS values in that interval are combined as a root mean square.
...
```typescript
// packages/vscode-extension/src/extension.ts:250-269
...
```typescript
// packages/vscode-extension/src/extension.ts:1156-1180
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:2269-2279
      const dslLines = [
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:2282-2283
      const start = await activeClient.call('start_engine', { debug: true })
      expect(start.isError, start.text).toBe(false)
```

```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:2339-2341
