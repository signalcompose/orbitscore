...
```typescript
// packages/vscode-extension/src/extension.ts:2216-2227
...
```typescript
// packages/vscode-extension/src/extension.ts:2234-2236
...
```typescript
// packages/vscode-extension/src/extension.ts:2237-2286 (setDocumentDirectory 注入前まで)
  } else {
...
```typescript
// packages/vscode-extension/src/extension.ts:2286-2309
    } else {
...
```typescript
// packages/vscode-extension/src/extension.ts:2314-2371
...
```typescript
// packages/vscode-extension/src/extension.ts:2583-2592
...
The quotation is indented by four rather than eight because on 2026-09-08 (#773, [PR #811](https://github.com/signalcompose/orbitscore/pull/811)) this branch moved out of a `for` loop and into a `createLinePrefixer` callback. Before that, stdout was only `output.split('\n')` per chunk, with **no partial line carried over to the next chunk**, so an `{"evalMark":...}` envelope cut at a chunk boundary produced a "malformed" warning for its first half while its second half slipped past the prefix tests and was discarded as an ordinary log line. **The entire evalMark reply is lost**, so the `await` in `evaluateForAgent()` does not return until it times out. The `savePluginState` / `pluginUi` / `engineState` branches share the path and break the same way. See the "The stdout bridge envelopes are reassembled into lines too" section of [IV-1](/en/editor/vscode-architecture) for the details.
...
```typescript
// packages/vscode-extension/src/extension.ts:3532-3606
...
```typescript
// packages/vscode-extension/src/extension.ts:3608-3619
...
```typescript
// packages/vscode-extension/src/diagnostics-analysis.ts:46-60
...
```typescript
// packages/engine/src/core/sequence.ts:110-113
...
```typescript
// packages/vscode-extension/src/extension.ts:3638-3649
...
```typescript
// packages/vscode-extension/src/extension.ts:3293-3310
...
```typescript
// packages/vscode-extension/src/extension.ts:3662-3679
