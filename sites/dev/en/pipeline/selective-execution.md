...
- **Spawning the engine puts the debug flag, the capture seam (#307) and `ORBIT_HOST_BUNDLE_ID` (#940) into env**. After the cutover, #377 had made it set `ORBITSCORE_ENGINE` explicitly, but [#840](https://github.com/signalcompose/orbitscore/pull/840) (#502) removed that env var along with the SC path. The Rust daemon is the only backend; the boot sequence of the `repl` subcommand is unchanged
...
The point is `stdio: ['pipe', 'pipe', 'pipe']`. Because stdin, stdout, and stderr are all pipe-connected, the extension can pump code in via `engineProcess.stdin.write(...)`. The `env` assembled in the omitted part carries the debug flag, the capture seam (#307) and `ORBIT_HOST_BUNDLE_ID` (#940 — the host bundle ID that lets a plugin window float in front); when the last cannot be resolved the key is deleted rather than passed through. `ORBITSCORE_ENGINE`, which used to announce the backend kind, was removed in #502 (see [0-2](/en/orientation/architecture-overview)). On receiving the `repl` subcommand, the engine calls `startREPLMode()`.
...
- `packages/vscode-extension/src/extension.ts:111-113` — declaration and purpose of the `globalInitialized` flag
- `packages/vscode-extension/src/engine-process.ts:302-453` — `startEngine()`: pre-check → env → process boot with `stdio: ['pipe','pipe','pipe']`
