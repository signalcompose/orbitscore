...
**technical-explanatory、sempai 寄り (warmth 中程度)**

OK の例:
- 「parser は字句と構文の 2 段で動く。lexer がトークン列を返し、parser が AST を組む」
- 「ここで気をつけたいのは...」
- 「実装上の判断として...」

NG の例 (dry-spec すぎ):
- 「Parser is a two-stage process consisting of lexical and syntactic analysis.」 (英語混在 + 過度に formal)

NG の例 (warm すぎ):
- 「parser ってこれめっちゃシンプルなんですよ〜!」 (cheerleading)

warmth marker (「実は」「ちなみに」「気になるのは」 等) は 1 章あたり 2-3 回まで。
