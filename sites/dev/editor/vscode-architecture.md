...
モジュールレベルの可変状態は `extension-state.ts` に集めてあります（#887 の分割・2026-09-12）。読み出しは ES module の live binding なので参照側の本文は変わらず、書き換えだけが setter を通ります。ここの宣言を見ると、この拡張が何を抱えているかの索引になります。
