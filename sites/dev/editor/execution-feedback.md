...
> **Note**: 本ページは 2026-09-01 時点での著者の reading の足跡で、2026-09-07 に #773（PR [#806](https://github.com/signalcompose/orbitscore/pull/806)・stdout の bridge dispatch の行単位化）まで追従しました。code が真実、本ページはその時点の理解の snapshot に過ぎません。
...
```typescript
// packages/vscode-extension/src/extension.ts:2807-2818
...
```typescript
// packages/vscode-extension/src/extension.ts:2825-2827
...
```typescript
// packages/vscode-extension/src/extension.ts:2828-2877 (setDocumentDirectory 注入前まで)
  } else {
...
```typescript
// packages/vscode-extension/src/extension.ts:2792-2805
...
```typescript
// packages/vscode-extension/src/extension.ts:2877-2900
    } else {
...
```typescript
// packages/vscode-extension/src/extension.ts:2964-2971
...
```typescript
// packages/vscode-extension/src/extension.ts:2905-2962
...
```typescript
// packages/vscode-extension/src/extension.ts:3168-3177
...
引用のインデントが 4 に見えるのは、この分岐が 2026-09-07 (#773・[PR #806](https://github.com/signalcompose/orbitscore/pull/806)) に `for` ループから `createLinePrefixer` のコールバックへ移ったからです。それ以前は stdout の chunk ごとに `output.split('\n')` するだけで**部分行を次の chunk へ持ち越していなかった**ので、`{"evalMark":...}` が chunk 境界で割れると前半は「malformed」警告になり、後半は prefix 判定をすり抜けて通常ログとして捨てられていました。**`evalMark` の応答がまるごと失われる**ので、`evaluateForAgent()` の `await` は timeout するまで返りません。`savePluginState` / `pluginUi` / `engineState` の 3 分岐も同じ経路なので同じ壊れ方をします。詳しくは [IV-1](/editor/vscode-architecture) の「stdout の bridge dispatch を行に戻す」節を参照してください。
...
```typescript
// packages/vscode-extension/src/extension.ts:4100-4174
...
```typescript
// packages/vscode-extension/src/extension.ts:4176-4187
...
面白いのは `undefined` を意図的に union から外している点です。変更前の signature は `string | undefined` で、`undefined` が「hardware バスへ流す」を意味していました。もし throw を素朴に `catch { return undefined }` へ書き換えていたら、エラー経路が**黙って hardware へ音を流す**ことになります。`hardware` と `skip` を別の variant にしておくと、その取り違えは型検査を通りません。
...
```typescript
// packages/vscode-extension/src/extension.ts:4230-4247
