...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:60-66
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:137-147
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:710-731
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:230-236
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:848-853
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:1389-1403
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:2026-2036
      const dslLines = [
...
```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:2039-2040
      const start = await activeClient.call('start_engine', { debug: true })
      expect(start.isError, start.text).toBe(false)
```

```typescript
// tests/e2e/orbitstudio-mcp-gated.spec.ts:2096-2098
...
- `tests/e2e/orbitstudio-mcp-gated.spec.ts:1-152` — env contract・stale artifact ガード
- `tests/e2e/orbitstudio-mcp-gated.spec.ts:332-609` — describe のセットアップ・`captureInstrumentScenario` の RMS ヘルパ・teardown
- `tests/e2e/orbitstudio-mcp-gated.spec.ts:611-1406` — 先頭テスト（起動・カタログ・capture・run_selection・onset 検証）
- `tests/e2e/orbitstudio-mcp-gated.spec.ts:2006-2112` — #654 playhead E2E
...
- `tests/e2e/gated-sources.ts:1-106` — ラチェットと衛生検査が共有する gated E2E ソース一覧（#668 PR-E1）
