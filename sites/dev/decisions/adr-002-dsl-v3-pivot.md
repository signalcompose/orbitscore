...
ちなみに現在も、この v3.0 は「audio engine line」として維持され、その上に Pitch DSL (MIDI 出力) が **別軸** で積まれています。`DSL_VERSION` は audio line の v3.0 ではなく DSL spec の版を指し、#883 の暗黙終端廃止で **2.0** になりました。拡張 (`.vsix` と git タグが名乗る版) は **4.2.1**、`ENGINE_VERSION` は **2.0.0** のままです — 3 つは同期しない別軸です (`docs/design/656-release-design.md` §4.4)。本 ADR は audio line の話です。
...
```typescript
// packages/vscode-extension/src/diagnostics-provider.ts:85-94
