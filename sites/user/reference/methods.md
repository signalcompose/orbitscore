...
| `chop(N)` | オーディオファイルを N 個のスライスに均等分割する。`chop(1)` と省略時は分割せず、元の長さ・元の音程のまま鳴ってスロットを越える | `arp.chop(8)` |
| `play(...)` | スライス番号でリズムパターンを定義する（0 は休符） | `kick.play(1, 0, 1, 0)` |
| `length(N)` | ループの長さを N 小節に設定する（`chop(2)` 以上のときは再生速度・音程も変わる） | `seq.length(2)` |
...
::: info pan() は音を小さくする方向にだけ働きます
`pan()` は、振った側をそのまま残して**反対側を減らす**ことで定位を作ります。振り切っても音が大きくなることはありません。`pan(-100)` なら右チャンネルが無音になり、左チャンネルは `pan(0)` のときと同じ大きさのままです。

そのため、パンを振るほど全体の音量は下がって聞こえます。元の音量感に戻したいときは `gain()` を足して補ってください。

この規則は 2026-09-13 に変わりました。それ以前は振り切ったときに音が約 1.4 倍（+3 dB）に持ち上がり、代わりにオーディオファイルの再生音量そのものが 1/1.4（-3 dB）下がっていました。今はどちらの補正もありません。**同じ譜面のオーディオ再生は約 1.4 倍（+3 dB）大きく鳴ります**ので、以前と同じ音量に揃えたい場合は `gain(-3)` を書き足してください。
:::
...
>
> 🔴 **配布版では LinkAudio の音声送出が動きません**（2026-09-10 時点）。宣言は受理されますが Live にチャンネルは現れません。**そのとき音がどうなるかは未決です** — 仕様は「ハードウェア出力へフォールバックし警告が 1 回出る」と書いていますが、実機測定では**音が出ず警告も出ませんでした**。LinkAudio を前提にした演奏はしないでください。詳細は [LinkAudio](../midi/link-audio.md) を参照してください。
...
- **同名の insert のうち 1 つだけ**を開きたいときは、DSL ではなく**楽譜のプラグイン名を右クリック** → `OrbitScore: Open Plugin UI` を使います（[エフェクト](../mixing/effects.md#右クリックで-1-つだけ開く)）。`ui("名前")` は一致を全部開くため、1 つだけを指す形は DSL にはありません。
