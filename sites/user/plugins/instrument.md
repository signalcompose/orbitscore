...
- 現状、instrument に対して名前を指定する `ui("名前")` 形は対象がありません（`seq.effect()` は次章のとおり audio シーケンス専用で、instrument シーケンスにはまだ per-sequence のエフェクトチェーンが無いためです）。開いた UI を閉じるには、パネルを直接閉じてください。

## v1 の制約（正直な開示）

- **`~`（デチューン）は使えません。** プラグイン経路には pitch bend / CC がまだ無いため、警告のうえスキップされます。
- **CC 制御・per-note expression・テンポ連動**は未実装です。
- **`seq.effect()` は instrument シーケンスに使えません**（audio シーケンス専用・[エフェクトを挿す](../mixing/effects.md)を参照）。
- **`sum` バスへの `output()` も instrument シーケンスには使えません。** instrument の出力は現状常に master へ直接ミックスされます。ただし `global.effect()`（マスターチェーン）は instrument の音にも適用されます。
