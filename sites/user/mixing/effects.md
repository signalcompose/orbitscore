...
同名のプラグインを 2 つ挿していて、**そのうちの 1 つだけ**を開きたいことがあります。`ui("名前")` は全部開くので、この用途には使えません。

そのときは楽譜の**プラグイン名の上にカーソルを置いて右クリック**し、`OrbitScore: Open Plugin UI` を選びます。**カーソルが乗っている 1 つだけ**が開きます。
...
- 右クリックはプラグイン名の**文字列の上**で行ってください。そこ以外だと「カーソルがプラグイン名の上にありません」というエラーが出ます。
- `Gain` のような標準プラグインの上で選ぶと「標準プラグインに UI はありません」と出ます。パラメータは楽譜側に書くためです。
- 保存済みの state ファイル名の上でも開けません。プラグイン名の上に置き直してください。
- どの場合も**黙って何も起きないということはありません**。理由がメッセージで出ます。

### 窓が OrbitStudio の手前に浮きます（macOS）

開いたプラグインの窓は、**OrbitStudio が前面にいる間だけ**エディタの手前に浮きます。他のアプリを前面にすると、そのアプリに被さりません。プラグインの窓そのものをクリックしている間も、手前に浮いたままです。
