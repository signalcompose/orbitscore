...
::: tip chop(1) はスロットに合わせて縮められません
`chop(1)`（および `chop()` を書かない場合）は、ファイルが **元の長さ・元の音程のまま**鳴ります。ファイルがスロット（1 つの音に割り当てられた時間）より長ければ、そのまま鳴り続けて次の音に重なります。

これは不具合ではなく、そういう機能です。長い小節の頭で叩く銅鑼やシンバルのように、**サンプル自身の長さが音楽的な意味を持つ**ワンショットは、この書き方で鳴らします。

```text
gong.beat(21 by 4).length(1)
gong.audio("EAF_Gong_05.wav").chop(1)   // 17.7 秒、元の音程のまま
gong.play(1, 0, 0, 0)                    // 10.0 秒ごとに鳴らす → 約 7.7 秒が次に重なる
```
:::
...
::: warning 音程が変わります（`chop(2)` 以上のときだけ）
`length()` は再生速度を変えるため、音程も連動して変わります。`length(2)` にすると約 1 オクターブ低く聞こえます。これはバグではなく仕様です。意図的に音程の変化を利用した表現もできます。

ただし、これが起きるのは **`chop(2)` 以上でスライスに分割しているとき**だけです。`chop(1)` や `chop()` なしの場合、スライスをスロットに詰める処理そのものが行われないため、`length()` を変えてもファイルは元の長さ・元の音程のまま鳴ります（変わるのは鳴らす間隔だけです）。
:::
...
| メソッド | 説明 |
|---|---|
| `chop(N)` | オーディオファイルを N 個のスライスに分割する（`chop(1)` は分割せず、元の長さ・元の音程のまま鳴らす） |
| `play(1, 2, …)` | スライス番号で再生順を指定（0 は休符） |
| `length(N)` | ループ長を N 小節に変更（`chop(2)` 以上のときは速度・音程も変わる） |
