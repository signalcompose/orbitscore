...
**IMPORTANT**: Detailed design and specification documentation is maintained in Japanese, as **HTML files**, in the `/docs` directory. Always refer to `/docs` for:

- **Documentation Index**: [`docs/core/INDEX.html`](docs/core/INDEX.html) - すべてのドキュメントの目次（必読）
- **DSL Specification**: [`docs/core/INSTRUCTION_ORBITSCORE_DSL.html`](docs/core/INSTRUCTION_ORBITSCORE_DSL.html) - 単一信頼情報源（Single Source of Truth）
- **Project Rules**: [`docs/core/PROJECT_RULES.html`](docs/core/PROJECT_RULES.html) - 開発ワークフロー、Git規則、コミット規約
- **Work Log**: [`docs/development/WORK_LOG.md`](docs/development/WORK_LOG.md) - 完全な開発履歴と技術的決定事項
- **Implementation Plan**: [`docs/development/IMPLEMENTATION_PLAN.html`](docs/development/IMPLEMENTATION_PLAN.html) - 技術ロードマップとフェーズ
- **Dev Learning Site Brief**: [`docs/development/DEV_LEARNING_SITE.html`](docs/development/DEV_LEARNING_SITE.html) - dev 学習サイト project brief + skill 運用 overrides
- **User Manual**: [`docs/user/ja/USER_MANUAL.html`](docs/user/ja/USER_MANUAL.html) - ユーザー向け機能説明 (日本語版)
- **Context7 Guide**: [`docs/core/CONTEXT7_GUIDE.html`](docs/core/CONTEXT7_GUIDE.html) - 外部ライブラリドキュメント参照ガイド
- **Testing Guide**: [`docs/testing/TESTING_GUIDE.html`](docs/testing/TESTING_GUIDE.html) - テスト手順とガイド

**Documentation Rules**:
1. **`/docs` 配下のドキュメントは原則 HTML 形式 (`.html`) で管理する** — ローカルで使う開発ドキュメントはブラウザで開いて参照する自己完結型 HTML として直接管理する。
   - 公開向け学習サイト (`sites/dev/`, `sites/user/`) は **VitePress による別管理**（Markdown ソース → HTML ビルド）であり、この HTML 直接管理ルールの対象外。
   - **例外: 高頻度の運用ログは Markdown で管理する** — `docs/development/WORK_LOG.md` のように **コミット毎に人手/エージェントが直接追記する**ログは、HTML 手編集が非実用的でタグ崩れの事故リスクが高いため Markdown を正とする。
   - **境界の目安**: 高頻度に追記する運用ログ系は **Markdown**。更新頻度の低い安定リファレンス（DSL 仕様 / PROJECT_RULES / INDEX / IMPLEMENTATION_PLAN / ROADMAP 等）と凍結アーカイブ（`docs/archive/`）は **HTML 直接管理**。
   - 安定リファレンスを新規追加・更新するときは `.html` で編集する（その種の文書では Markdown を新規作成しない）。
2. All documentation in `/docs` must be written in Japanese
3. When updating project design or specifications, update the `/docs` files (HTML、ただし運用ログ WORK_LOG は Markdown) accordingly
4. CLAUDE.md should remain concise and reference `/docs` for details
...
#### 必須ドキュメント（Readツール）
1. [`docs/core/PROJECT_RULES.html`](docs/core/PROJECT_RULES.html) - 開発ワークフロー、Git規則、重要ルール
2. [`docs/core/INDEX.html`](docs/core/INDEX.html) - ドキュメント構造の全体像とその下のファイルによる仕様や設計の確認
...
- Read("docs/core/PROJECT_RULES.html")
- Read("docs/core/INDEX.html")
その後、必要なメモリを並行で読み込む
```

#### タスク依存の追加ドキュメント
必要に応じて以下のドキュメントを読み込む（作業内容に応じて判断）:
- [`docs/core/INSTRUCTION_ORBITSCORE_DSL.html`](docs/core/INSTRUCTION_ORBITSCORE_DSL.html) - DSL仕様が必要な場合
- [`docs/development/IMPLEMENTATION_PLAN.html`](docs/development/IMPLEMENTATION_PLAN.html) - 実装計画の確認が必要な場合
- [`docs/development/WORK_LOG.md`](docs/development/WORK_LOG.md) - 過去の実装経緯を確認する場合
- [`docs/core/CONTEXT7_GUIDE.html`](docs/core/CONTEXT7_GUIDE.html) - 外部ライブラリドキュメントが必要な場合
- [`docs/testing/TESTING_GUIDE.html`](docs/testing/TESTING_GUIDE.html) - テスト手順が必要な場合
...
- **Work Log**: Every commit MUST be documented in [`docs/development/WORK_LOG.md`](docs/development/WORK_LOG.md)
...
# Edit docs/development/WORK_LOG.md
...
**OrbitScore 用の事前確定事項と運用 overrides** は [`docs/development/DEV_LEARNING_SITE.html`](docs/development/DEV_LEARNING_SITE.html) に集約する。

### 起動前の必須読み込み

`/vitepress-learning-site` または当該 skill を invoke する作業に入る前に、
必ず [`docs/development/DEV_LEARNING_SITE.html`](docs/development/DEV_LEARNING_SITE.html) を読み込むこと。
...
- **📝 [`docs/development/WORK_LOG.md`](docs/development/WORK_LOG.md)** - 開発履歴（技術的決定事項）
