...
- Product: 拡張 **4.0.1**（`.vsix` と git タグが担う版・正本は `packages/vscode-extension/package.json`）。
...
🔴 **`-t` で開発を回すのは正しいが、`-t` は「そのテストが後続を壊すこと」を原理的に検出できない**（2026-09-13・#917 で実測）。
`launchIsolatedOrbitStudio` は冒頭で `killHarnessInstances()` を呼ぶので、**共有セッションを使うテストより前に
自前アプリのテストを置くと、後続が `ECONNREFUSED` で落ちる**。単独実行では**自分だけ緑**になって見えない。

| 目的 | 走らせ方 |
|---|---|
| 自分のテストを直しながら回す | **`-t`**（自己完結テストのみ・実測 2 分） |
| **他テストへの影響を見る** | **全件**。`-t` では代替できない |
| 束 PR / main 直行 PR のマージ前ゲート | **全件** |

**新しい自前アプリのテストは、共有セッションを使うテスト群より後ろに置くこと**（`orbitstudio-mcp-gated.spec.ts` の
`#779 startup sweep` 以降がその領域）。
...
- 束は差分 **1,500 行以下**（🔴 **変更行 = `+` と `-` の合計**で数える・2026-09-07 #799）で、設計上の継ぎ目（wire / DSL・記録 / 再生・実時間 / オフライン）で切る。🔴 **上限より先に「検算の機会」で切る** — 振る舞いを変えない PR（互換維持の配線入れ替え）を、振る舞いを変える PR と同じ束に入れると「golden が動かないこと」で検証する機会が永久に失われる。旧経路の撤去は新表面が実機で確かめられた後の束へ（`BUNDLE_BRANCH_WORKFLOW.md` §5.1）。割り当ては plan §2.5
...
| 分割で module doc をコピペしない | `tests/repo/module-doc-purity.spec.ts`（#887） | 同一 doc を持つファイル組が増えたら red（ラチェット） |
