...
3. **全指摘を集約し、修正前に設計パスを置く。** エラー封じ込め・診断ポリシー・ガード方針などの
   ...
   **指摘単位のローカルパッチは禁止**（振動の主因）。
   ...
   🔴 **fixer は Codex を使う**（owner 訂正 2026-07-31）。ただし **fix の質は指示の質で決まる**ので、
   Fable に出すのと同じ密度のブリーフを書く — **ポリシーの明文化 / 具体的な故障シナリオ /
   変異検証の要求 / 検証コマンド / 「やってはいけないこと」**。「指摘を並べただけ」の発注はしない。

   **fix を Codex に最大3回出す。各 fix には次項の確認レビューが伴うので実質4ラウンド。**
   そこで収束しなければ **Fable を fixer 兼裁定者として投入**する。

   > 2026-07-30 に「ラウンド1から Fable」と定めたが撤回する。#591 の実測で **Fable を fixer に
   > しても1発では収束しなかった**（ラウンド1で Critical 2 + Important 4 を直した結果、
   > ラウンド2で **fix 起因の Important が2件**）。「高い fixer なら1ラウンドで終わる」は
   > 成立せず、token コストだけが増えた。
