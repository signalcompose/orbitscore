...
- **Work Log**: [`docs/development/WORK_LOG.html`](docs/development/WORK_LOG.html) - 完全な開発履歴と技術的決定事項
...
**Documentation Rules**:
1. **`/docs` 配下のドキュメントは HTML 形式 (`.html`) で管理する** — ローカルで使う開発ドキュメントはブラウザで開いて参照する自己完結型 HTML として直接管理し、Markdown ソースは残さない。
   - 公開向け学習サイト (`sites/dev/`, `sites/user/`) は **VitePress による別管理**（Markdown ソース → HTML ビルド）であり、この HTML 直接管理ルールの対象外。
   - 新しい `/docs` 文書を追加・更新するときも `.html` で編集する（Markdown を新規作成しない）。
2. All documentation in `/docs` must be written in Japanese
3. When updating project design or specifications, update the `/docs` HTML files accordingly
...
- [`docs/development/WORK_LOG.html`](docs/development/WORK_LOG.html) - 過去の実装経緯を確認する場合
