...
> **Step 2.3 実装完了 — Step 2.2 のベース上に sum-by-name (per-channel float mix accumulator + 1-tick deferred commit) を追加した状態。 同 channel への複数 Synth が tick 内で加算 mix される。 TS dispatch / boot pipeline 統合 (Step 4) は別 PR。**
...
| 2.3 | per-channel mix buffer + deferred commit (sum-by-name) | ✅ in this PR |
