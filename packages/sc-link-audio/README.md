...
> **Step 2.3 実装完了 + plugin follow-ups (Issue #205, #206) — Step 2.2 のベース上に sum-by-name (per-channel float mix accumulator + 1-tick deferred commit)、 `/cmd registerLinkAudioChannel` の OSC `/done` reply (DoAsynchronousCommand 経由)、 blockSize truncation test harness を追加した状態。 同 channel への複数 Synth が tick 内で加算 mix される。 TS dispatch / boot pipeline 統合 (Step 4) は別 PR。**

## 目的

- DSL `seq.output("channel-name")` で指定された sequence の出力を、 仮想ループバックデバイス無しで Live トラックに直結する
- macOS arm64 only (v1.x release target)
- 加算合成 (sum-by-name): 同名 channel への複数 Synth は `SinkEntry::mix.mixBuffer` (float accumulator) に tick 内で `+=` され、 次 tick 先頭で flush + commit される (Step 2.3、 issue #200)。 1-tick の commit latency が発生する代わりに last-write-wins regression を回避
...
| 2.2 | `OrbitLinkAudioOut` UGen の単一 channel commit 実装 | ✅ |
| 2.3 | per-channel mix buffer + deferred commit (sum-by-name) | ✅ |
| 2.4 | 同名 sum 動作の検収 (Live UI 実機確認) | ⏳ TBD (`verify-live-receive.scd` の `test-sum` channel で operator 検収) |
| 2.5 | tempo / phase / transport sync (LinkAudio 内蔵 Link) | ⏳ TBD (実装済、 検収は Step 4 と統合 E2E で) |
| 205 | `/cmd registerLinkAudioChannel` OSC `/done` reply | ✅ in this PR (DoAsynchronousCommand 経由) |
| 206 | blockSize truncation test harness | ✅ in this PR (`scripts/verify-blocksize-truncation.scd`) |
