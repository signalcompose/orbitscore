 ...
 * 唯一の契約面。`RustEnginePlayer`（orbit-audio-daemon / WebSocket）がこれを満たす。
 *
 ...
 *   - cutover #108 で既定を Rust（native daemon）に切替。旧 SC バックエンドは
 *     #502 で削除済み — 唯一のバックエンドが Rust daemon。
 */
 ...
 * とデバイス・LinkAudio 面を持つ。`boot` は optional な device 引数を受ける
 * （`AudioEngine.boot()` より広い）。
 */
export interface AudioEngineBackend extends Scheduler {
  ...
  /** daemon GetStatus の構造化スナップショット（Rust REPL bridge / MCP 可観測性用）。 */
